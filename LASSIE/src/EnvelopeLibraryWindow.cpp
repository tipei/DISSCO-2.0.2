...
EnvelopeLibraryWindow::EnvelopeLibraryWindow() {
    ...
    */
    ...
    attributesRefBuilder->get_widget("ScaleLabel", ScaleBoundary);
    ...
    m_refActionGroup->add(Gtk::Action::create("ContextAdd", "Create New Envelope"),
                          sigc::mem_fun(*this, &EnvelopeLibraryWindow::createNewEnvelope));

    m_refActionGroup->add(Gtk::Action::create("ContextDuplicate", "Duplicate Envelope"),
    ...
    Gtk::Main::signal_key_snooper().connect(
        sigc::mem_fun(*this, &EnvelopeLibraryWindow::captureKeyStroke));
...
void EnvelopeLibraryWindow::refreshEnvelopeList() {
    // this method is very inefficient, but for the first version we'll do this
    // for now.
...
void EnvelopeLibraryWindow::objectActivated(const Gtk::TreeModel::Path& _path,
...
void EnvelopeLibraryWindow::on_cursor_changed() {
    ...
    drawingArea->adjustBoundary(row[columns.columnEntry]);
...
int EnvelopeLibraryWindow::captureKeyStroke(Gtk::Widget* _widget, GdkEventKey* _gdkEventKey) {
    if (_gdkEventKey->type == 8 && _gdkEventKey->keyval == 115 &&
...
void EnvelopeLibraryWindow::duplicateEnvelope() {
    ...
    EnvelopeLibraryEntry* newEnvelope = activeProject->duplicateEnvelope(originalEnvelope);
    activeProject->modified();
    Gtk::TreeModel::Row childrow = *(refTreeModel->append());
