...
EnvLibDrawingArea::EnvLibDrawingArea(EnvelopeLibraryWindow* _envelopeLibraryWindow) {
    ...
    add_events(Gdk::POINTER_MOTION_MASK | Gdk::ENTER_NOTIFY_MASK | Gdk::BUTTON_PRESS_MASK |
               Gdk::BUTTON_RELEASE_MASK);

    signal_motion_notify_event().connect(sigc::mem_fun(*this, &EnvLibDrawingArea::mouseMoving));
    signal_button_press_event().connect(
        sigc::mem_fun(*this, &EnvLibDrawingArea::mouseButtonPressed));
    signal_button_release_event().connect(
        sigc::mem_fun(*this, &EnvLibDrawingArea::mouseButtonReleased));
    ...
    m_refActionGroup->add(Gtk::Action::create("ContextAdd", "Insert Node"),
                          sigc::mem_fun(*this, &EnvLibDrawingArea::insertEnvelopeSegment));
    m_refActionGroup->add(Gtk::Action::create("ContextRemove", "RemoveNode"),
                          sigc::mem_fun(*this, &EnvLibDrawingArea::removeNode));

    m_refActionGroup->add(Gtk::Action::create("ContextSetFlexible", "Set Flexible"),
                          sigc::mem_fun(*this, &EnvLibDrawingArea::setFlexible));
    m_refActionGroup->add(Gtk::Action::create("ContextSetFixed", "Set Fixed"),
                          sigc::mem_fun(*this, &EnvLibDrawingArea::setFixed));

    m_refActionGroup->add(Gtk::Action::create("ContextSetLinear", "Set Linear"),
                          sigc::mem_fun(*this, &EnvLibDrawingArea::setLinear));
    m_refActionGroup->add(Gtk::Action::create("ContextSetSpline", "Set Spline"),
                          sigc::mem_fun(*this, &EnvLibDrawingArea::setSpline));
    m_refActionGroup->add(Gtk::Action::create("ContextSetExponential", "Set Exponential"),
...
bool EnvLibDrawingArea::mouseMoving(GdkEventMotion* event) {
    ...
    // check boundaries
    if (y > upperY) y = upperY;
    if (y < lowerY) y = lowerY;
    char xs[10];
    char ys[10];
...
bool EnvLibDrawingArea::mouseButtonPressed(GdkEventButton* event) {
    ...
    while (true) {
        if (mouseX >= (nodeX - 5) && mouseX <= (nodeX + 5) && mouseY >= (nodeY - 5) &&
            mouseY <= (nodeY + 5)) {
...
void EnvLibDrawingArea::showGraph(EnvelopeLibraryEntry* _envelope) {
    ...
    if (window) {
        ...
        while (segment != NULL) {  // draw this segment
            ...
            double endy =
                height - (getAdjustedY(segment->rightNode->y) * height * height / (height + 1));
            ...
            if (segment->segmentType == envSegmentTypeExponential) {
                ...
                // 1 + ((y(t) - y1)/y1) = (1 + ((y2 - y1)/y1)) ^ ((t - t1)/(t2 -
                // t1))
                double tempx, tempy, step = 10, ostarty, oendy,
                                     oy;  // oy,ostarty,oendy is the original unscaled y that
                                          // need to be tranlated to our system
                ...
                if (exponential == 0) {
                    // opposite curve
                    tempx = startx;
                    tempy = starty * pow((1 + (endy - starty) / starty),
                                         ((tempx - startx) / (endx - startx)));
                    while (tempx < endx) {
                    ...
                    ostarty = endy;
                    oendy = starty;
                    oy = ostarty * pow((1 + (oendy - ostarty) / ostarty),
                                       ((tempx - startx) / (endx - startx)));
            ...
            // mi = (y_i+1-y_i-1)/(x_i+1-x_i-1) for i=2,...,n-1 which is just
            // the average of the slopes surrounding a point, and for the ends:
            ...
            if (segment->rightNode->rightSeg == NULL)
              ...
              double y3 =
            height-(getAdjustedY(segment->rightNode->rightSeg->rightNode->y)*height*height
            / (height + 1)); double x3 =
            segment->rightNode->rightSeg->rightNode->x *width * width /
            (width+1); m2 = (y3 - endy -1) / (x3 - endx -1);
            }
            double a = endy;
            double b = m1;
            double c = 3 * (slope - 2*m1 - m2) / (endx - startx);
            double d = (m1 + m2 - 2*slope) / ((endx - startx) * (endx -
            startx)); tempx = startx; tempy = starty; while (tempx < endx){
              cr->move_to(tempx,tempy);
              tempx += step;
              tempy = a + b*(tempx - startx) + c*pow((tempx-startx),2) +
            d*pow(tempx-startx,3); cr->line_to(tempx, tempy); cr->stroke();
            }*/
...
void EnvLibDrawingArea::moveNode() {
    ...
    if (activeNode->leftSeg == NULL || activeNode->rightSeg == NULL) {  // head and tail
        ...
        activeNode->x =
            (mouseX >= leftBound) ? ((mouseX <= rightBound) ? mouseX : rightBound) : leftBound;
        activeNode->y = mouseY;
    }
...
void EnvLibDrawingArea::adjustBoundary(EnvelopeLibraryEntry* _envelope) {
    ...
    if (max > upperY) upperY = max;
    if (min < lowerY) lowerY = min;
...
double EnvLibDrawingArea::mouseAdjustY(double y) { return y * (upperY - lowerY) + lowerY; }
