...
FunctionGenerator::FunctionGenerator(FunctionReturnType _returnType, std::string _originalString) {
   ...
   */
    ...
    else if (_returnType == functionReturnSPE) {
        Gtk::TreeModel::Row row = *(functionListTreeModel->append());
        row[functionListColumns.m_col_id] = functionSpectrum_Gen;
        row[functionListColumns.m_col_name] = "Generate_Spectrum";
    ...
    // Random
    attributesRefBuilder->get_widget("RandomLowBoundFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomLowBoundFunButtonClicked));

    attributesRefBuilder->get_widget("RandomHighBoundFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomHighBoundFunButtonClicked));
    ...
    attributesRefBuilder->get_widget("RandomIntLowBoundFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomIntLowBoundFunButtonClicked));

    attributesRefBuilder->get_widget("RandomIntHighBoundFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomIntHighBoundFunButtonClicked));

    attributesRefBuilder->get_widget("RandomIntLowBoundEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomIntEntryChanged));
    ...
    attributesRefBuilder->get_widget("RandomizerBaseFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomizerBaseFunButtonClicked));

    attributesRefBuilder->get_widget("RandomizerDeviationFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomizerDeviationFunButtonClicked));

    attributesRefBuilder->get_widget("RandomizerBaseEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomizerEntryChanged));
    ...
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::randomDensityEntryChanged));
    ...
    attributesRefBuilder->get_widget("DecayBaseFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::decayBaseFunButtonClicked));

    attributesRefBuilder->get_widget("DecayRateFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::decayRateFunButtonClicked));

    attributesRefBuilder->get_widget("DecayIndexFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::decayIndexFunButtonClicked));

    attributesRefBuilder->get_widget("DecayTypeExponentialRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::decayTypeRadioButtonClicked));
    ...
    attributesRefBuilder->get_widget("InverseFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::inverseFunButtonClicked));
    ...
    entry->signal_changed().connect(sigc::mem_fun(*this, &FunctionGenerator::markovEntryChanged));

    // LN

    attributesRefBuilder->get_widget("LNFunButton", button);
    ...
    attributesRefBuilder->get_widget("FibonacciFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::FibonacciFunButtonClicked));

    attributesRefBuilder->get_widget("FibonacciEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::FibonacciEntryChanged));
    // select

    attributesRefBuilder->get_widget("SelectIndexFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::selectIndexFunButtonClicked));
    ...
    attributesRefBuilder->get_widget("SelectAddNodeButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::selectAddNodeButtonClicked));

    attributesRefBuilder->get_widget("SelectInsertFunctionButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::selectFunButtonClicked));
    ...
    */
    ...
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::stochosMethodRadioButtonClicked));
    ...
    attributesRefBuilder->get_widget("StochosAddNodeButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::stochosAddNodeButtonClicked));

    attributesRefBuilder->get_widget("StochosInsertFunctionButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::stochosFunButtonClicked));

    // ValuePick

    attributesRefBuilder->get_widget("ValuePickElementsMeaningfulRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::valuePickTextChanged));
    ...
    attributesRefBuilder->get_widget("ValuePickAbsRangeFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::valuePickAbsRangeFunButtonClicked));

    attributesRefBuilder->get_widget("ValuePickLowFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::valuePickLowFunButtonClicked));

    attributesRefBuilder->get_widget("ValuePickHighFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::valuePickHighFunButtonClicked));

    attributesRefBuilder->get_widget("ValuePickDistFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::valuePickDistFunButtonClicked));

    // ChooseL

    attributesRefBuilder->get_widget("ChooseLFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::chooseLFunButtonClicked));
    ...
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::getPatternEntryChanged));
    ...
    attributesRefBuilder->get_widget("GetPatternOffsetFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::getPatternOffsetFunButtonClicked));

    attributesRefBuilder->get_widget("GetPatternOffsetEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::getPatternEntryChanged));

    attributesRefBuilder->get_widget("GetPatternFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::getPatternFunButtonClicked));
    ...
    attributesRefBuilder->get_widget("MakeListFunctionFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeListFunctionFunButtonClicked));

    attributesRefBuilder->get_widget("MakeListSizeFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeListSizeFunButtonClicked));
    ...
    attributesRefBuilder->get_widget("EnvLibEnvelopeFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::envLibEnvelopeFunButtonClicked));

    attributesRefBuilder->get_widget("EnvLibScalingFactorFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::envLibScalingFactorFunButtonClicked));
    ...
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::readENVFileTextChanged));

    // MakeEnvelope

    attributesRefBuilder->get_widget("MakeEnvelopeScalingFactorFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeEnvelopeScalingFactorFunButtonClicked));

    attributesRefBuilder->get_widget("MakeEnvelopeXValueFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeEnvelopeXValueFunButtonClicked));

    attributesRefBuilder->get_widget("MakeEnvelopeYValueFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeEnvelopeYValueFunButtonClicked));

    attributesRefBuilder->get_widget("MakeEnvelopeScalingFactorEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeEnvelopeTextChanged));
    ...
    attributesRefBuilder->get_widget("MakePatternIntervalsFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makePatternIntervalsFunButtonClicked));

    attributesRefBuilder->get_widget("MakePatternIntervalsEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makePatternTextChanged));

    // ExpandPattern

    attributesRefBuilder->get_widget("ExpandPatternEquivalenceRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::expandPatternRadioButtonClicked));
    ...
    attributesRefBuilder->get_widget("ExpandPatternModuloFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::expandPatternModuloFunButtonClicked));

    attributesRefBuilder->get_widget("ExpandPatternLowFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::expandPatternLowFunButtonClicked));
    attributesRefBuilder->get_widget("ExpandPatternHighFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::expandPatternHighFunButtonClicked));
    attributesRefBuilder->get_widget("ExpandPatternPatternFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::expandPatternPatternFunButtonClicked));

    attributesRefBuilder->get_widget("ExpandPatternModuloEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::expandPatternTextChanged));
    ...
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::readPATFileTextChanged));

    // ReadFILFile
    attributesRefBuilder->get_widget("ReadFILFileEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::readFILFileTextChanged));

    // MakeFilter
    attributesRefBuilder->get_widget("MakeFilterFrequencyEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterTextChanged));
    ...
    attributesRefBuilder->get_widget("MakeFilterFrequencyFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterFrequencyFunButtonClicked));

    attributesRefBuilder->get_widget("MakeFilterBandWidthFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterBandWidthFunButtonClicked));

    attributesRefBuilder->get_widget("MakeFilterDBGainFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterDBGainFunButtonClicked));

    attributesRefBuilder->get_widget("MakeFilterLPFRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterTextChanged));

    attributesRefBuilder->get_widget("MakeFilterHPFRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterTextChanged));

    attributesRefBuilder->get_widget("MakeFilterBPFRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterTextChanged));

    attributesRefBuilder->get_widget("MakeFilterLSFRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterTextChanged));

    attributesRefBuilder->get_widget("MakeFilterHSFRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterTextChanged));

    attributesRefBuilder->get_widget("MakeFilterNFRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterTextChanged));

    attributesRefBuilder->get_widget("MakeFilterPBEQFRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeFilterTextChanged));
    ...
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::readREVFileTextChanged));

    // REV_Simple
    attributesRefBuilder->get_widget("REV_SimpleEntryFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_SimpleEntryFunButtonClicked));

    attributesRefBuilder->get_widget("REV_SimpleEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_SimpleEntryTextChanged));

    // REV_Medium

    attributesRefBuilder->get_widget("REV_MediumReverbPercentFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_MediumReverbPercentFunButtonClicked));

    attributesRefBuilder->get_widget("REV_MediumHilowSpreadFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_MediumHilowSpreadFunButtonClicked));

    attributesRefBuilder->get_widget("REV_MediumGainAllPassFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_MediumGainAllPassFunButtonClicked));

    attributesRefBuilder->get_widget("REV_MediumDelayFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_MediumDelayFunButtonClicked));

    attributesRefBuilder->get_widget("REV_MediumReverbPercentEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_MediumTextChanged));
    ...
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_AdvancedReverbPercentFunButtonClicked));

    attributesRefBuilder->get_widget("REV_AdvancedCombGainListFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_AdvancedCombGainListFunButtonClicked));

    attributesRefBuilder->get_widget("REV_LPGainListFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_AdvancedLPGainListFunButtonClicked));

    attributesRefBuilder->get_widget("REV_AdvancedGainAllPassFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_AdvancedGainAllPassFunButtonClicked));

    attributesRefBuilder->get_widget("REV_AdvancedDelayFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_AdvancedDelayFunButtonClicked));

    attributesRefBuilder->get_widget("REV_AdvancedReverbPercentEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::REV_AdvancedTextChanged));
    ...
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::readSIVFileTextChanged));

    // MakeSieve

    attributesRefBuilder->get_widget("MakeSieveElementsMeaningfulRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeSieveTextChanged));
    ...
    attributesRefBuilder->get_widget("MakeSieveLowFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeSieveLowFunButtonClicked));

    attributesRefBuilder->get_widget("MakeSieveHighFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::makeSieveHighFunButtonClicked));

    // ReadSPAFile

    attributesRefBuilder->get_widget("ReadSPAFileEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::readSPAFileTextChanged));

    // SPA
    attributesRefBuilder->get_widget("SPAStereoRadioButton", radiobutton);
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SPAMethodRadioButtonClicked));
    ...
    radiobutton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SPAApplyByRadioButtonClicked));
    ...
    // Spectrum_Gen-generating spectrum based on distance

    attributesRefBuilder->get_widget("Spectrum_GenDistanceFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::Spectrum_GenDistanceFunButtonClicked));

    attributesRefBuilder->get_widget("Spectrum_GenEnvelopeFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::Spectrum_GenEnvelopeFunButtonClicked));

    attributesRefBuilder->get_widget("Spectrum_GenDistanceEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::Spectrum_GenTextChanged));
    ...
    if (functionName.compare("Stochos") == 0) {
        ...
        if (functionFlag == 1) {  // functions
            while (envelopeElement != NULL) {
                ...
                Gtk::VBox* vbox;
                attributesRefBuilder->get_widget("StochosInnerVBox", vbox);
    ...
    if (functionName.compare("Select") == 0) {
        ...
        if (listElements.size() > 1 || listElements[0].compare("") != 0) {
            ...
            for (functionString = listElements.begin(); functionString != listElements.end();
                ...
                Gtk::VBox* vbox;
                attributesRefBuilder->get_widget("SelectInnerVBox", vbox);
                vbox->pack_start(*newSubAlignment, Gtk::PACK_SHRINK);
        ...
        /*
            ...
            // first argument is a list (in XML it's just a string, format:
           a,b,c,d... thisElement =
           functionNameElement->getNextElementSibling();
    ...
    if (functionName.compare("ValuePick") == 0) {
        ...
        thisElement = thisElement->getNextElementSibling();  // second argument is an
                                                             // envelope (low)
        attributesRefBuilder->get_widget("ValuePickLowEntry", entry);
        entry->set_text(getFunctionString(thisElement));

        thisElement = thisElement->getNextElementSibling();  // third argument is an
                                                             // envelope (high)
        attributesRefBuilder->get_widget("ValuePickHighEntry", entry);
        entry->set_text(getFunctionString(thisElement));

        thisElement = thisElement->getNextElementSibling();  // fourth argument is an
                                                             // envelope (distribution)
        attributesRefBuilder->get_widget("ValuePickDistEntry", entry);
        entry->set_text(getFunctionString(thisElement));

        thisElement = thisElement->getNextElementSibling();  // 5th argument is a string
                                                             // (Elements)
        ...
        thisElement = thisElement->getNextElementSibling();  // 6th argument is a list
                                                             // without bracket
        ...
        thisElement = thisElement->getNextElementSibling();  // 8th argument is a list
                                                             // without bracket

        attributesRefBuilder->get_widget("ValuePickWeightsEntry", entry);
        entry->set_text(getFunctionString(thisElement));

        thisElement = thisElement->getNextElementSibling();  // 9th argument is a string
                                                             // (Elements)
...
std::string FunctionGenerator::getResultString() { return result; }
        ...
        if (row) {
            ...
            if (function == functionRandom) {
                ...
                textview->get_buffer()->set_text(
                    "<Fun><Name>Random></Name><Low>0</Low><High>1</High></"
                    "Fun>");
                attributesRefBuilder->get_widget("RandomLowBoundEntry", entry);
                entry->set_text("0");
                ...
                // reset all data
                textview->get_buffer()->set_text(
                    "<Fun><Name>RandomInt</Name><LowBound>0</"
                    "LowBound><HighBound>1</HighBound></Fun>");
                attributesRefBuilder->get_widget("RandomIntLowBoundEntry", entry);
                entry->set_text("0");
                ...
                // reset all data
                textview->get_buffer()->set_text(
                    "<Fun><Name>Randomizer</Name><Base></Base><Deviation></"
                    "Deviation></Fun>");
                attributesRefBuilder->get_widget("RandomizerBaseEntry", entry);
                entry->set_text("");
                ...
                alignment->add(*vbox);
                textview->get_buffer()->set_text(
                    "<Fun><Name>RandomDensity</Name><Envelope></"
                    "Envelope><Low></Low><High></High></Fun>");
                ...
                // reset all data
                textview->get_buffer()->set_text(
                    "<Fun><Name>Fibonacci</Name><Entry></Entry></Fun>");
                attributesRefBuilder->get_widget("FibonacciEntry", entry);
                entry->set_text("");
                set_position(Gtk::WIN_POS_CENTER_ALWAYS);
                resize(400, 300);
                ...
                textview->get_buffer()->set_text(
                    "<Fun><Name>Decay</Name><Base></Base><Type>EXPONENTIAL</"
                    "Type><Rate></Rate><Index>CURRENT_PARTIAL_NUM</Index></"
                    "Fun>");
                attributesRefBuilder->get_widget("DecayBaseEntry", entry);
                entry->set_text("");
                ...
                textview->get_buffer()->set_text(
                    "<Fun><Name>Stochos</Name><Method>RANGE_DISTRIB</"
                    "Method><Envelopes></Envelopes><Offset></Offset></Fun>");
                attributesRefBuilder->get_widget("StochosOffsetEntry", entry);
                entry->set_text("");
                ...
                textview->get_buffer()->set_text(
                    "<Fun><Name>Select</Name><List></List><Index></Index></"
                    "Fun>");
                attributesRefBuilder->get_widget("SelectIndexEntry", entry);
                entry->set_text("");
                set_position(Gtk::WIN_POS_CENTER_ALWAYS);
            ...
            } else if (function == functionSpectrum_Gen) {
                alignment->remove();
                attributesRefBuilder->get_widget("Spectrum_GenVBox", vbox);
                alignment->add(*vbox);
...
void FunctionGenerator::randomIntEntryChanged() {
    ...
    std::string stringbuffer =
        "<Fun><Name>RandomInt</Name><LowBound>" + entry->get_text() + "</LowBound><HighBound>";
...
void FunctionGenerator::randomizerEntryChanged() {
    ...
    attributesRefBuilder->get_widget("RandomizerBaseEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>Randomizer</Name><Base>" + entry->get_text() + "</Base><Deviation>";
...
void FunctionGenerator::decayTypeRadioButtonClicked() { decayTextChange(); }
void FunctionGenerator::decayEntryChanged() { decayTextChange(); }
...
void FunctionGenerator::decayTextChange() {
    ...
    std::string stringbuffer =
        "<Fun><Name>Decay</Name><Base>" + entry->get_text() + "</Base><Type>";
...
void FunctionGenerator::inverseEntryChanged() {
    ...
    attributesRefBuilder->get_widget("InverseEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>Inverse</Name><Entry>" + entry->get_text() + "</Entry></Fun>";
    textview->get_buffer()->set_text(stringbuffer);
}
...
void FunctionGenerator::markovEntryChanged() {
    ...
    Gtk::Entry* entry;
    attributesRefBuilder->get_widget("MarkovEntry", entry);
    textview->get_buffer()->set_text("<Fun><Name>Markov</Name><Entry>" + entry->get_text() +
                                     "</Entry></Fun>");
}
...
void FunctionGenerator::FibonacciEntryChanged() {
    ...
    attributesRefBuilder->get_widget("FibonacciEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>Fibonacci</Name><Entry>" + entry->get_text() + "</Entry></Fun>";
    textview->get_buffer()->set_text(stringbuffer);
}
...
FunctionGenerator::SelectSubAlignment::SelectSubAlignment(FunctionGenerator* _parent) {
...
#ifdef GLIBMM_EXCEPTIONS_ENABLED
    try {
        attributesRefBuilder->add_from_file("./LASSIE/src/UI/FunGenSelectSubAlignment.ui");
    } catch (const Glib::FileError& ex) {
        std::cerr << "FileError: " << ex.what() << std::endl;
    } catch (const Gtk::BuilderError& ex) {
        std::cerr << "BuilderError: " << ex.what() << std::endl;
    ...
    attributesRefBuilder->get_widget("removeNodeButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SelectSubAlignment::removeButtonClicked));

    attributesRefBuilder->get_widget("FunctionsEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SelectSubAlignment::textChanged));
    entry->set_text("");
}
...
FunctionGenerator::StochosSubAlignment::StochosSubAlignment(FunctionGenerator* _parent,
    ...
    attributesRefBuilder->get_widget("removeNodeButton1", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::StochosSubAlignment::removeButtonClicked));

    attributesRefBuilder->get_widget("removeNodeButton2", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::StochosSubAlignment::removeButtonClicked));

    attributesRefBuilder->get_widget("FunctionsEntry", entry);
    entry->signal_changed().connect(
        sigc::mem_fun(*this, &FunctionGenerator::StochosSubAlignment::textChanged));
    entry->set_text("ENV");
...
void FunctionGenerator::valuePickTextChanged() {
    ...
    attributesRefBuilder->get_widget("ValuePickAbsRangeEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>ValuePick</Name><Range>" + entry->get_text() + "</Range><Low>";
    ...
    attributesRefBuilder->get_widget("ValuePickElementsEntry", entry);
    stringbuffer =
        stringbuffer + "</Method><Elements>" + entry->get_text() + "</Elements><WeightMethod>";
...
void FunctionGenerator::chooseLEntryChanged() {
    ...
    attributesRefBuilder->get_widget("ChooseLEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>ChooseL</Name><Entry>" + entry->get_text() + "</Entry></Fun>";
    textview->get_buffer()->set_text(stringbuffer);
}
...
void FunctionGenerator::makeListTextChanged() {
    ...
    attributesRefBuilder->get_widget("MakeListFunctionEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>MakeList</Name><Func>" + entry->get_text() + "</Func><Size>";
...
FunctionGenerator::RawListHBox::RawListHBox(FunctionGenerator*
_parentGenerator){ parentGenerator = _parentGenerator; attributesRefBuilder =
Gtk::Builder::create(); #ifdef GLIBMM_EXCEPTIONS_ENABLED try{
...
void FunctionGenerator::envLibTextChanged() {
    ...
    attributesRefBuilder->get_widget("EnvLibEnvelopeEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>EnvLib</Name><Env>" + entry->get_text() + "</Env><Scale>";
...
void FunctionGenerator::readENVFileTextChanged() {
    ...
    attributesRefBuilder->get_widget("ReadENVFileEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>ReadENVFile</Name><File>" + entry->get_text() + "</File></Fun>";
...
FunctionGenerator::MakeEnvelopeSubAlignment::MakeEnvelopeSubAlignment(FunctionGenerator* _parent) {
    ...
    attributesRefBuilder->get_widget("XValueFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::MakeEnvelopeSubAlignment::xValueFunButtonClicked));

    attributesRefBuilder->get_widget("YValueFunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::MakeEnvelopeSubAlignment::yValueFunButtonClicked));

    attributesRefBuilder->get_widget("InsertNodeButton", button);
    button->signal_clicked().connect(sigc::mem_fun(
        *this, &FunctionGenerator::MakeEnvelopeSubAlignment::insertNodeButtonClicked));

    attributesRefBuilder->get_widget("RemoveNodeButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::MakeEnvelopeSubAlignment::removeButtonClicked));
...
void FunctionGenerator::makePatternTextChanged() {
    ...
    attributesRefBuilder->get_widget("MakePatternIntervalsEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>MakePattern</Name><List>" + entry->get_text() + "</List></Fun>";
    textview->get_buffer()->set_text(stringbuffer);
}

void FunctionGenerator::expandPatternRadioButtonClicked() { expandPatternTextChanged(); }
...
void FunctionGenerator::expandPatternTextChanged() {
    ...
    attributesRefBuilder->get_widget("ExpandPatternEquivalenceRadioButton", radiobutton);
    attributesRefBuilder->get_widget("ExpandPatternSymmetriesRadioButton", radiobutton2);
...
void FunctionGenerator::readPATFileTextChanged() {
    ...
    attributesRefBuilder->get_widget("ReadPATFileNameEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>ReadPATFile</Name><File>" + entry->get_text() + "</File></Fun>";
...
void FunctionGenerator::readFILFileTextChanged() {
    ...
    attributesRefBuilder->get_widget("ReadFILFileEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>ReadFILFile</Name><File>" + entry->get_text() + "</File></Fun>";
...
void FunctionGenerator::readREVFileTextChanged() {
    ...
    attributesRefBuilder->get_widget("ReadREVFileEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>ReadREVFile</Name><File>" + entry->get_text() + "</File></Fun>";
...
void FunctionGenerator::REV_SimpleEntryTextChanged() {
    ...
    attributesRefBuilder->get_widget("REV_SimpleEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>REV_Simple</Name><Size>" + entry->get_text() + "</Size></Fun>";
...
void FunctionGenerator::REV_MediumTextChanged() {
    ...
    attributesRefBuilder->get_widget("REV_MediumReverbPercentEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>REV_Medium</Name><Percent>" + entry->get_text() + "</Percent><Spread>";
...
void FunctionGenerator::REV_AdvancedTextChanged() {
    ...
    attributesRefBuilder->get_widget("REV_AdvancedReverbPercentEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>REV_Advanced</Name><Percent>" + entry->get_text() + "</Percent><CombGainList>";
...
void FunctionGenerator::readSIVFileTextChanged() {
    ...
    attributesRefBuilder->get_widget("ReadSIVFileEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>ReadSIVFile</Name><File>" + entry->get_text() + "</File></Fun>";
...
void FunctionGenerator::makeSieveTextChanged() {
    ...
    attributesRefBuilder->get_widget("MakeSieveLowEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>MakeSieve</Name><Low>" + entry->get_text() + "</Low><High>";
    ...
    attributesRefBuilder->get_widget("MakeSieveWeightsEntry", entry);
    stringbuffer =
        stringbuffer + "</WeightMethod><Weight>" + entry->get_text() + "</Weight><Offset>";
...
void FunctionGenerator::readSPAFileTextChanged() {
    ...
    attributesRefBuilder->get_widget("ReadSPAFileEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>ReadSPAFile</Name><File>" + entry->get_text() + "</File></Fun>";
...
void FunctionGenerator::Spectrum_GenTextChanged() {
    ...
    attributesRefBuilder->get_widget("Spectrum_GenEnvelopeEntry", entry);
    std::string stringbuffer =
        "<Fun><Name>Spectrum_Gen</Name><Envelope>" + entry->get_text() + "</Envelope><Distance>";

    attributesRefBuilder->get_widget("Spectrum_GenDistanceEntry", entry);
    stringbuffer = stringbuffer + entry->get_text() + "</Distance></Fun>";

    textview->get_buffer()->set_text(stringbuffer);
}

FunctionGenerator::SPAPartialAlignment::SPAPartialAlignment(FunctionGenerator* _parent,
    ...
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SPAPartialAlignment::removePartialButtonClicked));

    attributesRefBuilder->get_widget("InsertButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SPAPartialAlignment::insertPartialButtonClicked));

    attributesRefBuilder->get_widget("FunButton", button);
    button->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SPAPartialAlignment::funButtonClicked));
...
void FunctionGenerator::SPAPartialAlignment::textChanged() { parent->SPATextChanged(); }
...
FunctionGenerator::SPAChannelAlignment::SPAChannelAlignment(FunctionGenerator* _parent,
                                                            int _numOfPartial,
    ...
    insertButton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SPAChannelAlignment::insertChannelButtonClicked));

    attributesRefBuilder->get_widget("RemoveChannelButton", removeButton);
    removeButton->signal_clicked().connect(
        sigc::mem_fun(*this, &FunctionGenerator::SPAChannelAlignment::removeChannelButtonClicked));
...
int FunctionGenerator::SPAChannelAlignment::getButtonsShownStatus() { return flagButtonsShown; }
...
std::string FunctionGenerator::getFunctionString(FileValue* _value,
...
list<std::string> FunctionGenerator::fileValueListToStringList(list<FileValue> _valueList,
                                                               FunctionReturnType _returnType) {
