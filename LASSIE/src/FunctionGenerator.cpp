 ...
 */
  ...
  */
  ...
  attributesRefBuilder->get_widget(
    "Spectrum_GenDistanceFunButton", button);
  button->signal_clicked().connect(sigc::mem_fun(
    *this, &FunctionGenerator::Spectrum_GenDistanceFunButtonClicked));

  attributesRefBuilder->get_widget(
    "Spectrum_GenEnvelopeFunButton", button);
  button->signal_clicked().connect(sigc::mem_fun(
    *this, &FunctionGenerator::Spectrum_GenEnvelopeFunButtonClicked));

  attributesRefBuilder->get_widget(
    "Spectrum_GenDistanceEntry", entry);
  entry->signal_changed().connect(sigc::mem_fun(
    *this, & FunctionGenerator::Spectrum_GenTextChanged));

  attributesRefBuilder->get_widget(
    "Spectrum_GenEnvelopeEntry", entry);
  entry->signal_changed().connect(sigc::mem_fun(
    *this, & FunctionGenerator::Spectrum_GenTextChanged));
...
void FunctionGenerator::Spectrum_GenTextChanged(){
  ...
  attributesRefBuilder->get_widget(
    "Spectrum_GenEnvelopeEntry", entry);
  std::string stringbuffer = "<Fun><Name>Spectrum_Gen</Name><Envelope>" + entry->get_text() + "</Envelope><Distance>";
