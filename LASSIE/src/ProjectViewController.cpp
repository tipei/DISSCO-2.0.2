...
ProjectViewController::ProjectViewController(std::string _pathAndName, MainWindow* _mainWindow) {
    ...
    // make a new shared pointers object so all object can find each other
    // easily without calling layers of layers of parents/ children
...
void ProjectViewController::hideContents() { cleanUpContents(); }
...
void ProjectViewController::insertObject() {
    ...
    if (selectedPaletteFolder == "None") {
        Gtk::MessageDialog dialog("Please select a folder to store the new object.",
                                  false /* use_markup */, Gtk::MESSAGE_QUESTION, Gtk::BUTTONS_OK);
    ...
    if (result == 1) {
        ...
        if (selectedPaletteFolder != flagForFolderMatching) {
            ...
            stringBuffer =
                "The folder you selected to store the object is \"" + selectedPaletteFolder + "\".";
            ...
            stringBuffer =
                "The object you want to create has the type \"" + flagForFolderMatching + "\".";
            ...
            stringBuffer = "Put this object in the \"" + flagForFolderMatching + "\" folder.";
            ...
            stringBuffer = "Change the type of the object to \"" + selectedPaletteFolder + "\".";
        ...
        if (decision == 0) {
            ...
            for (it = events.begin(); it != events.end(); it++) {
                if ((*it)->getEventName() == nameEntry->get_text() &&
                    ...
                    refBuilder2->add_from_file("./LASSIE/src/UI/CheckObjectNameExists.ui");
...
std::string ProjectViewController::getPathAndName() { return pathAndName; }

//----------------------------------------------------------------------------//

void ProjectViewController::newObjectButtonClicked() { newObjectDialog->hide(); }
...
void ProjectViewController::save() {
    ...
    */
    ...
    stringBuffer =
        synthesis ? "    <Synthesis>True</Synthesis>\n" : "    <Synthesis>False</Synthesis>\n";
    fputs(stringBuffer.c_str(), file);
...
PaletteViewController* ProjectViewController::getPalette() { return paletteView; }
...
vector<MarkovModel<float>>& ProjectViewController::getMarkovModels() { return markovModels; }
...
ProjectViewController::ProjectViewController(MainWindow* _mainWindow, std::string _pathAndName,
    ...
    // if measure !=0,there is a measure entry in .dissco file, parse next
    // element else, take the previous parse as the one for piece start time
    if (measure != "0") {
...
EnvelopeLibraryEntry* ProjectViewController::convertToLASSIEEnvLibEntry(Envelope* _envelope,
                                                                        int index) {
...
bool ProjectViewController::getEmptyProject() { return emptyProject; }
...
bool ProjectViewController::getSaved() { return !modifiedButNotSaved; }
...
void ProjectViewController::configureNoteModifiers() {
    ...
    // Get the GtkBuilder-instantiated Dialog:
    noteModifiersConfigurationDialogRefBuilder->get_widget("NoteConfigurationDialog",
                                                           noteModifiersConfigurationDialog);
    noteModifiersConfigurationDialog->set_title("Note Modifiers Configuration");

    noteModifiersConfigurationDialogRefBuilder->get_widget("CustomModifiersVBox",
    ...
    if (defaultNoteModifiers["-8va"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("Minus8vaButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["+8va"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("Plus8vaButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["bend"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("BendButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["dry"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("DryButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["glissKeys"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("GlissKeysButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["glissStringRes"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("GlissStringResButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["graceTie"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("GraceTieButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["letVibrate"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("LetVibrateButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["moltoVibrato"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("MoltoVibratoButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["mute"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("MuteButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["pedal"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("PedalButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["pluck"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("PluckButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["pressSilently"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("PressSilentlyButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["resonance"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("ResonanceButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["resPedal"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("ResPedalButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["sfz"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("SfzButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["sffz"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("SffzButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["tenuto"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("TenutoButton", checkButton);
        checkButton->set_active(true);
    }

    if (defaultNoteModifiers["tremolo"]) {
        noteModifiersConfigurationDialogRefBuilder->get_widget("TremoloButton", checkButton);
        checkButton->set_active(true);
    }
    ...
    Gtk::Button* button;
    noteModifiersConfigurationDialogRefBuilder->get_widget("AddModifierButton", button);

    button->signal_clicked().connect(
        sigc::mem_fun(*this, &ProjectViewController::ConfigureNoteModifiersAddButtonClicked));
...
CustomNoteModifierHBox::CustomNoteModifierHBox(ProjectViewController* _projectView) {
    ...
    removeButton.signal_clicked().connect(
        sigc::mem_fun(*this, &CustomNoteModifierHBox::removeButtonClicked));
}
...
std::string CustomNoteModifierHBox::getText() { return entry.get_text(); }

//---------------------------------------------------------------------------//

void CustomNoteModifierHBox::removeButtonClicked() { projectView->removeCustomNoteModifier(this); }
