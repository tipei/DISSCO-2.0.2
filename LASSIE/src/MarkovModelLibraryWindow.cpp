...
MarkovModelLibraryWindow::MarkovModelLibraryWindow() {
    ...
    m_detailBox.pack_start(*Gtk::manage(new Gtk::Label("Initial Distribution", 0, 0)), false,
                           false);
    ...
    m_sizeButton.signal_clicked().connect(
        sigc::mem_fun(*this, &MarkovModelLibraryWindow::onSetSize));
    ...
    m_refActionGroup->add(Gtk::Action::create("ContextAdd", "Create New Model"),
                          sigc::mem_fun(*this, &MarkovModelLibraryWindow::createNewModel));

    m_refActionGroup->add(Gtk::Action::create("ContextDuplicate", "Duplicate Model"),
...
void MarkovModelLibraryWindow::buildTable() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            hbox->pack_start(*entryBox, false, false);
            entryBox->add(*entry);
            m_matrixEntries.push_back(entry);  // cache the entry into a
                                               // temporary vector for reference
        }
    }
