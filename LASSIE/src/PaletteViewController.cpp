...
int PaletteViewController::slot_comp_func(const Gtk::TreeModel::iterator& a,
                                          const Gtk::TreeModel::iterator& b) {
    int cmp = (*a).get_value(columns.columnObjectOrder) - (*b).get_value(columns.columnObjectOrder);
    if (cmp == 0)
        return (*a)
            .get_value(columns.columnObjectName)
            .compare((*b).get_value(columns.columnObjectName));
    if (cmp > 0) return 1;
    return -1;
}
...
PaletteViewController::PaletteViewController(SharedPointers* _sharedPointers)
    ...
    */
    ...
    palette.enable_model_drag_source(projectView->listTargets, Gdk::MODIFIER_MASK,
                                     Gdk::ACTION_COPY);
    ...
    m_refActionGroup->add(Gtk::Action::create("ContextDelete", "Delete"),
                          sigc::mem_fun(*this, &PaletteViewController::deleteObject));

    m_refActionGroup->add(Gtk::Action::create("ContextDuplicate", "Duplicate Object"),
    ...
    Glib::RefPtr<Gtk::TreeSelection> refTreeSelection = palette.get_selection();
...
void PaletteViewController::on_button_drag_data_get(const Glib::RefPtr<Gdk::DragContext>& context,
...
bool PaletteViewController::onRightClick(GdkEventButton* event) {
    ...
    palette.get_path_at_pos((int)event->x, (int)event->y, path, temp_column, x, y);
...
void PaletteViewController::deleteObject() {
    ...
    if (result != "") {
        ...
        Gtk::MessageDialog dialog(prompt, false /* use_markup */, Gtk::MESSAGE_QUESTION,
...
void PaletteViewController::duplicateObject() {
    ...
    string prompt = "You are about to duplicate this object:\n\n" +
                    originalEvent->getEventTypeString() + "/" + originalEvent->getEventName() +
    ...
    if (response == 1 && nameEntry->get_text() == "") {
        ...
        if (sharedPointers->projectView->checkNameExists(nameEntry->get_text(),
                                                         originalEvent->getEventType())) {
            Gtk::MessageDialog dialog("Object with the same name exists.", false /* use_markup */,
                                      Gtk::MESSAGE_QUESTION, Gtk::BUTTONS_OK);
