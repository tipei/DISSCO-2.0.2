...
string PieceHelper::getFixedPath(string path) {
    if (path == "") return "./";
    if (path.c_str()[path.length() - 1] != '/') path = path + "/";
    return path;
}
...
string PieceHelper::getProjectName(string path) {
    ...
    if (g == "") {
        ...
        for (unsigned int i = 0; i < files.size(); i++) cout << files[i] << endl;
        cout << "=========================================================" << endl;
        cout << endl;
    }
...
int PieceHelper::getSeedNumber(string seed) {
    ...
    int seedNumber = 0;
    int digits = 1;
    for (int i = 0; i < seed.length(); i++) digits *= 10;
...
Piece::Piece(string _workingPath, string _projectTitle) {
    ...
    utilities = new Utilities(root, _workingPath, soundSynthesis, outputParticel, numThreads,
                              numChannels, sampleRate, this);
    ...
    Tempo mainTempo;  // Though we supply this, "Top" will provide its own
                      // tempo.
  ...
  //}*/
    ...
    Event* topEvent =
        new Event(topElement, pieceSpan, 0, mainTempo, utilities, NULL, NULL, NULL, NULL);
    ...
    if (type <= 4) {  // Top, High, Medium, Low, Bottom
        ...
        if (definitionVal == 0) {  // Only Continuum
            ...
            if (XMLTC(flagElement) == "0") {  // Continuum
                ...
                numChildren =
                    (int)(soundsPsec * utilities->evaluate(pieceDuration, NULL) + underOne / area);

            } else {  // by layer
                numChildren = 0;
                for (int i = 0; i < layerElements.size(); i++) {
                    numChildren +=
                        utilities->evaluate(XMLTC(layerElements[i]->GFEC()), (void*)this);
                }
            }
            ...
            if (type == 4) {  // Bottom
                ...
                if (flagVal == 0) {  // Equal Temperament
                    ...
                    if (utilities->evaluate(XMLTC(continuumFlagElement),
                                            NULL) == 0) {  // Hertz
                        ...
                        double baseFreqResult =
                            pow(2, step * range) * MINFREQ;  // equal chance for all 8vs
                    }
                }

                /*entropy = calculateEntropyRatio(samples, "Pow2", 0, 15000);
                //Freq in Hz currentEntropy = utilities->eventValues[name];
                utilities->eventValues.erase(name);
                newEntropy = (currentEntropy + entropy); // Mean of ratios
                utilities->eventValues.insert(std::pair<string, double> (name,
                newEntropy));
                ...
                for (int i = 0; i < numChildren; i++) {
                    ...
                    EventType childEventType = (EventType)utilities->evaluate(
                        XMLTC(childTypeElements[childType]->GFEC()->GNES()), (void*)this);

                    DOMElement* childElement =
                        utilities->getEventElement(childEventType, childName);
...
void Piece::geneticOptimization(string fitnessFunction, double optimum) {
    // Step 1: Calculating current Aesthetic of the piece - User decides
    // function
    string evName = utilities->topEventnames.at(0);
    EventType evType = (EventType)0;
    utilities->currChild = 0;
    ...
    for (std::map<string, double>::iterator it = utilities->eventValues.begin();
         it != utilities->eventValues.end(); ++it) {
    ...
    if (pieceAesthetic < optimum) {
        ...
        for (int i = 0; i < prange.size(); i++) {
            if (prange[i] >= randNum) {
                ...
                child.push_back(
                    utilities->getEventElement(t, sortedEvents[sortedEvents.size() - i]));
                sortedEvents.erase(sortedEvents.begin() + i);
                runningSum = 0.0;
                prange.clear();
                break;
...
void Piece::crossoverMutation(DOMElement* parent1, DOMElement* parent2, DOMElement* child,
                              double mutationProb) {
...
vector<DOMElement*> Piece::modifyPiece(DOMElement* eventElement) {
    ...
    /*if(type <= 4){  //Top, High, Medium, Low, Bottom
      ...
      thisEventElement = thisEventElement->GNES(); //maxChildDur
      float maxChildDur = (float)utilities->evaluate(XMLTC(thisEventElement),
    (void*)this);

      thisEventElement = thisEventElement->GNES(); //newEDUPerBeat
      int newEDUPerBeat = (int)
    utilities->evaluate(XMLTC(thisEventElement),(void*)this);
      ...
      DOMElement* numChildrenElement = thisEventElement->GNES(); //Num children
    element
      ...
      if (XMLTC(flagElement) =="0"){ // Continuum
        ...
        if (XMLTC(entry1Element)==""){
          ...
          numChildren =(int) utilities->evaluate(XMLTC(entry1Element),
    (void*)this);
        }
      }
      else if (XMLTC(flagElement) == "1"){ // Density
        ...
        DOMElement* areaElement = densityElement->GNES();
        DOMElement* underOneElement = areaElement->GNES();
        double density = utilities->evaluate(
    XMLTC(densityElement),(void*)this); double area = utilities->evaluate(
    XMLTC(areaElement),(void*)this); double underOne = utilities->evaluate(
    XMLTC(underOneElement),(void*)this); double soundsPsec = pow(2, density *
    area - underOne); //this can't be right..
    //  cout<<"density:"<< density<<", area:"<<area<<",
    underOne:"<<underOne<<endl;

        //not sure which version is the correct one. ask sever
        //Check if this is correct!!!
        numChildren = (int)(soundsPsec * utilities->evaluate(pieceDuration,
    NULL) + underOne/area);
    ...
    if (type == 4) {  // Bottom
        ...
        if (loudnessElement->GFEC() != NULL) {
            ...
            // char *loudnessvalue =
            // XMLString::transcode(loudnessElement->getFirstChild()->getNodeValue());
    ...
    /*for(int i = 0; i < numChildren; i++){
      ...
      double childType =
  utilities->evaluate(XMLTC(childTypeElement),(void*)this); string childName =
  XMLTC(childTypeElements[childType]->GFEC()); EventType childEventType =
  (EventType)
  utilities->evaluate(XMLTC(childTypeElements[childType]->GFEC()->GNES()),(void*)this);

      DOMElement* childElement = utilities->getEventElement(childEventType,
  childName); childElements.push_back(childElement);
...
// Experimental
void Piece::functionModifier(DOMElement* functionElement,
                             int maxValue) {  // Needs some range info
    ...
    if (functionName.compare("RandomInt") == 0) {
        ...
        if (highBoundElement->GFEC() != NULL) {
            ...
            highBound =
                (highBound + Random::RandInt(0, maxValue / 2)) % (maxValue / 2) + maxValue / 2;
            char val[33];
        ...
        if (highBoundElement->GFEC() != NULL) {
            ...
            highBound =
                fmod((highBound + Random::Rand(0, maxValue / 2)), maxValue / 2) + maxValue / 2;
            char val[33];
...
vector<DOMElement*> Piece::calculateAesthetic(DOMElement* eventElement) {
    ...
    if (type <= 4) {  // Top, High, Medium, Low, Bottom
        ...
        if (definitionVal == 0) {  // Only Continuum
            ...
            entropy = calculateEntropyRatio(samples, "Unit", 0.0,
            atoi(pieceDuration.c_str())); //duration currentEntropy =
            utilities->eventValues[name]; utilities->eventValues.erase(name);
            ...
            if (type == 4) {  // Bottom
                ...
                entropy = calculateEntropyRatio(samples, "Pow2", 0,
                                                15000);  // Freq in Hz
...
double Piece::calculateEntropyRatio(vector<double> sampleData, string partitionMethod, double min,
                                    double max) {
