...
Utilities::Utilities(DOMElement* root, string _workingPath, bool _soundSynthesis,
                     bool _outputParticel, int _numThreads, int _numChannels, int _samplingRate,
                     Piece* _piece)
    ...
    if (tagName != "MarkovModelLibrary") {
        cout << "Project is outdated, please save the project in the latest "
                "version of DISSCO"
             << endl;
        exit(1);
    }
    ...
    while (thisEventElement) {
        ...
        switch (type) {
            case 0:
                topEventElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                topEventnames.insert(pair<int, string>(topCounter, eventName));

                eventValues.insert(pair<string, double>(eventName, 0.0));

                break;
            case 1:
                highEventElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                highEventnames.insert(pair<int, string>(highCounter, eventName));

                eventValues.insert(pair<string, double>(eventName, 0.0));

                break;
            case 2:
                midEventElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                midEventnames.insert(pair<int, string>(midCounter, eventName));

                eventValues.insert(pair<string, double>(eventName, 0.0));

                break;
            case 3:
                lowEventElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                lowEventnames.insert(pair<int, string>(lowCounter, eventName));

                eventValues.insert(pair<string, double>(eventName, 0.0));

                break;
            case 4:
                bottomEventElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                bottomEventnames.insert(pair<int, string>(bottomCounter, eventName));

                eventValues.insert(pair<string, double>(eventName, 0.0));

                break;
            case 5:
                spectrumElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                spectrumEventnames.insert(pair<int, string>(spectrumCounter, eventName));

                eventValues.insert(pair<string, double>(eventName, 0.0));

                break;
            case 6:
                envelopeElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                envelopeEventnames.insert(pair<int, string>(envelopeCounter, eventName));
                break;
            case 7:
                sieveElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                sieveEventnames.insert(pair<int, string>(sieveCounter, eventName));
                break;
            case 8:
                ...
                break;
            case 9:
                patternElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                patternEventnames.insert(pair<int, string>(patternCounter, eventName));
                break;
            case 10:
                reverbElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                reverbEventnames.insert(pair<int, string>(reverbCounter, eventName));
                break;
            case 12:
                notesElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                notesEventnames.insert(pair<int, string>(notesCounter, eventName));
                break;
            case 13:
                filterElements.insert(pair<string, DOMElement*>(eventName, thisEventElement));

                filterEventnames.insert(pair<int, string>(filterCounter, eventName));
                break;
        }
...
double Utilities::evaluate(std::string _input, void* _object) {
    ...
    while (locOfFun != string::npos) {
        ...
        functionStringLength =
            ((int)locOfEndFun) + 6 - ((int)locOfFun);  // 6 is the length of "</fun>"
...
bool Utilities::isSieveFunction(DOMElement* input) {
    return ((input = input->GFEC()) && (input = input->GFEC()) &&
            (XMLTC(input) == "ChooseL" || XMLTC(input) == "ValuePick"));
}

//----------------------------------------------------------------------------//

void* Utilities::evaluateObject(string _input, void* _object, EventType _returnType) {
    // remove any spaces
...
std::vector<std::string> Utilities::listElementToStringVector(DOMElement* _listElement) {
    ...
    while (!doneProcessingListString) {
        ...
        while (!listElementLocated) {
            if (locationOfComma == string::npos) {  // reach the end of the string
                ...
                size_t endOfThisFun =
                    Utilities::findTheEndOfFirstFunction(listString.substr(locationOfFun)) +
                    locationOfFun;
...
string Utilities::evaluateFunction(string _functionString, void* _object) {
    ...
    xercesc::MemBufInputSource myxml_buf((const XMLByte*)_functionString.c_str(),
                                         _functionString.size(), "function (in memory)");
...
string Utilities::static_function_CURRENT_TYPE(void* _object) {
    if (_object != NULL) {
        ...
        cerr << "Utilities:Warning! static_function_CURRENT_TYPE has no object "
                "to look up."
             << endl;
        return "0";
    }
...
string Utilities::static_function_CURRENT_CHILD_NUM(void* _object) {
    if (_object != NULL) {
        ...
        cerr << "Utilities:Warning! static_function_CURRENT_NUM has no object "
                "to look up."
             << endl;
        return "0";
    }
...
string Utilities::static_function_CURRENT_PARTIAL_NUM(void* _object) {
    if (_object != NULL) {
        ...
        cerr << "Utilities:Warning! static_function_CURRENT_PARTIAL_NUM has no "
                "object to look up."
             << endl;
        return "0";
    }
...
string Utilities::static_function_CURRENT_DENSITY(void* _object) {
    cout << "Utilities:Warning! static_function_CURRENT_DENSITY is not "
            "implemented in CMOD 2.0 yet."
         << endl;
}
...
string Utilities::static_function_CURRENT_SEGMENT(void* _object) {
    ...
    cout << "Utilities:Warning! static_function_CURRENT_SEGMENT is not "
            "implemented in CMOD 2.0 yet."
         << endl;
}
...
string Utilities::static_function_AVAILABLE_EDU(void* _object) {
    if (_object != NULL) {
        ...
        cerr << "Utilities:Warning! static_function_AVAILABLE_EDU has no "
                "object to look up."
             << endl;
        return "0";
    }
...
string Utilities::static_function_CURRENT_LAYER(void* _object) {
    if (_object != NULL) {
        ...
        cerr << "Utilities:Warning! static_function_CURRENT_LAYER has no "
                "object to look up."
             << endl;
        return "0";
    }
...
string Utilities::static_function_PREVIOUS_CHILD_DURATION(void* _object) {
    if (_object != NULL) {
        ...
        cerr << "Utilities:Warning! static_function_PREVIOUS_CHILD_DURATION "
                "has no object to look up."
             << endl;
        return "0";
    }
}
...
string Utilities::function_Stochos(DOMElement* _functionElement, void* _object) {
    ...
    if (method == "FUNCTIONS") {
        ...
        // distribution within given range; takes 3 envs: min, MAX, val in
        // between
        if (3 * offset >= envVect.size()) {
            cerr << "Error - Stochos - Not enough envelopes on the list: "
                    "envVect.size="
        ...
        for (int i = 0; i < 2; i++) {
            if (envVect[3 * offset + i] != NULL) {
                ...
                cerr << "Stochos - NULL envelope. Trying to access envy[" << 3 * offset + i
                     << "]=" << envVect[3 * offset + i] << endl;
            }
        }
        ...
        if (envVect[3 * offset + 2]) {
            ...
            cerr << "Stochos - NULL envelope. Trying to access envVect[" << 3 * offset + 2
                 << "]=" << envVect[3 * offset + 2] << endl;
        }
...
Sieve* Utilities::sieve_ValuePick(DOMElement* _functionElement, void* _object) {
    ...
    Sieve si;
    if (eMethod == "MODS") {
        si.BuildFromExpr(minVal, maxVal, eMethod.c_str(), wMethod.c_str(), eArgs[0], wArgVect,
                         offsetVect);
    } else {
...
string Utilities::function_RandomDensity(DOMElement* _functionElement, void* _object) {
    DOMElement* envelopeNumberElement =
        _functionElement->getFirstElementChild()->getNextElementSibling();
    ...
    // cout << "lowbound: " << lowBound << ", highbound: " << highBound << ",
    // result: " << resultNumber << endl;
...
Sieve* Utilities::getSieve(string _functionString, void* _object) {
    ...
    xercesc::MemBufInputSource myxml_buf((const XMLByte*)toParse.c_str(), toParse.size(),
                                         "function (in memory)");
...
Sieve* Utilities::getSieveHelper(void* _object, DOMElement* _SIVFunction) {
    ...
    // those values.
        ...
        if (offsetString.size() == 1) {  // only one number, so we need to copy it to form a vector
                                         // to match the number elements
            for (int i = 0; i < eArgs.size(); i++) {
        ...
        Sieve* siv = new Sieve();
        if (eMethod == "MODS") {
            siv->BuildFromExpr(minVal, maxVal, eMethod.c_str(), wMethod.c_str(), eArgs[0], wArgInts,
                               offsetVect);
        } else {
    ...
    // on the new function.
        ...
        string selectedListElementString =
            "<root>" + function_SelectObject(_SIVFunction->GFEC(), _object) + "</root>";

        XercesDOMParser* parser = new XercesDOMParser();

        xercesc::MemBufInputSource myxml_buf((const XMLByte*)selectedListElementString.c_str(),
...
Patter* Utilities::getPatternHelper(void* _object, DOMElement* _PATFunction) {
    ...
    // those values.
        ...
        return newPattern;  // Utilities will put this pattern in _object's
                            // patternStorage
    ...
    // on the new function.
        ...
        string selectedListElementString =
            "<root>" + function_SelectObject(_PATFunction->GFEC(), _object) + "</root>";
...
DOMElement* Utilities::getSPAFunctionElementHelper(void* _object, DOMElement* _SPAFunction,
                                                   bool _initialCall) {
    ...
    if (XMLTranscode(functionNameElement).compare("ReadSPAFile") == 0) {
        ...
        if (select == string::npos) {
            ...
            string selectedListElementString =
                "<root><Fun><Name>ReadSPAFile</Name><File>" +
                function_SelectObject(SPAElement->GFEC()->GFEC()->GNES()->GFEC(), _object) +
                "</File></Fun></root>";
            // Here we need to push a temporary parser in the _object to stored
            // the parsed selectedListElementString...
            ...
            // after the statement above, the ownership of the parser is
            // transfered to
            // the _object. the _object is responsible to clean up the memory of
            // the parser, which is done by the std::vector automatically.
        ...
        string selectedListElementString =
            "<root>" + function_SelectObject(SPAElement->GFEC(), _object) + "</root>";
        ...
        ((Event*)_object)->addTemporaryXMLParser(parser);
        // after the statement above, the ownership of the parser is transfered
        // to
...
DOMElement* Utilities::getREVFunctionElementHelper(void* _object, DOMElement* _REVFunction,
                                                   bool _initialCall) {
    ...
    if (XMLTranscode(functionNameElement).compare("ReadREVFile") == 0) {
        ...
        if (select == string::npos) {
            ...
            // cout<<functionString<<endl;
            string selectedListElementString =
                "<root><Fun><Name>ReadREVFile</Name><File>" +
                function_SelectObject(REVElement->GFEC()->GFEC()->GNES()->GFEC(), _object) +
                "</File></Fun></root>";
            // Here we need to push a temporary parser in the _object to stored
            // the parsed selectedListElementString...
        ...
        string selectedListElementString =
            "<root>" + function_SelectObject(REVElement->GFEC(), _object) + "</root>";
...
DOMElement* Utilities::getFILFunctionElementHelper(void* _object, DOMElement* _FILFunction,
                                                   bool _initialCall) {
    ...
    if (XMLTranscode(functionNameElement).compare("ReadFILFile") == 0) {
        ...
        if (select == string::npos) {
            ...
            // cout<<functionString<<endl;
            string selectedListElementString =
                "<root><Fun><Name>ReadFILFile</Name><File>" +
                function_SelectObject(FILElement->GFEC()->GFEC()->GNES()->GFEC(), _object) +
                "</File></Fun></root>";
            // Here we need to push a temporary parser in the _object to stored
            // the parsed selectedListElementString...
        ...
        string selectedListElementString =
            "<root>" + function_SelectObject(FILElement->GFEC(), _object) + "</root>";
...
Envelope* Utilities::getEnvelope(string _input, void* _object) {
    XercesDOMParser* parser = new XercesDOMParser();
    xercesc::MemBufInputSource myxml_buf((const XMLByte*)_input.c_str(), _input.size(),
                                         "function (in memory)");
...
Envelope* Utilities::readEnvFile(DOMElement* _functionElement, void* _object) {
    ...
    DOMElement* builder =
        file->getFirstElementChild()->getNextElementSibling()->getNextElementSibling();
...
Envelope* Utilities::makeEnvelope(DOMElement* _functionElement, void* _object) {
    ...
    while (t != NULL && p != NULL) {
        ...
        if (XMLTranscode(p).compare("FIXED") == 0) {
            ...
            cerr << "Error in MakeEnvelope: Unrecognized envelope length "
                    "stretch type."
                 << endl;
            exit(1);
        }
