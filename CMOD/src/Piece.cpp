...
Piece::Piece(string _workingPath, string _projectTitle) {
    for (int i = 0; i < 1; i++) {
        ...
        // Convert seed string to seed number and seed the random number
        // generator
        ...
        for(std::map<string, double>::iterator it =
        utilities->eventValues.begin(); it != utilities->eventValues.end();
        ++it){ names.push_back(it->first); if(it->second != 0.0){ pieceAesthetic
        += it->second; numUniqueEvents++; bottoms.insert(std::pair<double,
        string>(it->second, it->first)); cout<<it->first<<",
        "<<it->second<<endl; fitnessSum += it->second;
          }
        }
