...
public:
    ...
    bool isDeterminate(void) { return (d != 0); }

    bool isWhole(void) { return (d == 1); }
    ...
    static Rational<T> Mod(Rational<T> a, Rational<T> b) {
        T new_d = LCM(a.d, b.d);
        T a_mult = new_d / a.d;
        T b_mult = new_d / b.d;
        a.n *= a_mult;
    ...
    Rational<T> operator+(Rational<T> other) {
        if (!isDeterminate() || !other.isDeterminate()) return Rational<T>(0, 0);
        T lcd = LCM(d, other.d);
        T new_d = lcd;
    ...
    Rational<T> operator+(T other) { return *this + Rational<T>(other, 1); }
    ...
    Rational<T> operator-() { return (*this * -1); }

    bool operator==(Rational<T> other) { return (n == other.n && d == other.d); }

    bool operator==(T other) { return (n == other && d == 1); }
    ...
    bool operator>(T other) {
        Rational<T> difference = *this - Rational<T>(other, 1);
        return (difference.n > 0);
    }
    ...
    Rational(const char* str) { *this = fromString(string(str)); }

    Rational(std::string str) { *this = fromString(str); }

    Rational<T> operator+(const std::string& other) { return *this + Rational<T>(other); }
    ...
    static Rational<T> fromString(const std::string& str) {
        ...
        for (T i = 0; i < len; i++) {
            ...
            if (c == '-') {
                ...
                if (c >= 48 && c <= 57) numerator = numerator * 10 + (T)(c - 48);
            } else {
                if (c >= 48 && c <= 57) denominator = denominator * 10 + (T)(c - 48);
            }
        }

        if (!isPastSlash) denominator = 1;
        if (isNegative) numerator = -numerator;

        return Rational<T>(numerator, denominator);
    }

    operator const char*(void) { return toPrettyString().c_str(); }
