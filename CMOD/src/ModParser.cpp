...
ModParser::Token::Token(int n, int minVal, int maxVal, int offset)
    : n(n), minVal(minVal), maxVal(maxVal), offset(offset), hasList(false) {}

ModParser::Token::Token(std::list<int> l) : l(l), hasList(true) {}
...
int ModParser::Token::getInt() { return n; }
...
std::list<int> ModParser::listComplement(const std::list<int>& l, int minVal, int maxVal) {
    ...
    for (std::list<int>::const_iterator it = l.begin();
         it != l.end() && elem <= *it && elem <= maxVal; ++it) {
...
void ModParser::parseOperator(std::vector<ModParser::Token>& operands, std::stack<char>& operators,
...
void ModParser::parseExpr(const std::string& exp, int minVal, int maxVal) {
    ...
    while (chNum < exp.size()) {
        ...
        if (isdigit(ch)) {
            ...
            while (!operators.empty() && precedence(operators.top()) != -1 &&
