...
vector<FileValue*> FileValue::EvaluateArgs(int n, ...) {
    ...
    if (ftnArgs.size() != n) {
        cerr << "FileValue::EvaluateArgs - Wrong # of arguments passed to " << ftnString
             << ", expected:" << n << ", got:" << ftnArgs.size() << endl;
        if (evptr != NULL) cerr << "      (in file " << evptr->getEventName() << ")" << endl;
        exit(1);
    }
    ...
    while (iter != ftnArgs.end()) {
        ...
        if (arg_type != iter->getReturnType()) {
            cerr << "FileValue::EvaluateArgs - Argument " << argNum << " of " << ftnString
                 << " has incorrect return_type (return_type= " << iter->getReturnType() << ")"
                 << endl;
            if (evptr != NULL) cerr << "      (in file " << evptr->getEventName() << ")" << endl;
            exit(1);
        }
...
void FileValue::Evaluate() {
    ...
    } else if (ftnString == "DURATION_UNITS") {  // Deprecated--use
                                                 // AVAILABLE_EDU
    ...
    /*  //check if the inner args of the function are functions themselves
        // ... and if they're not, make sure this function doesn't get
       recalculated
    ...
    if (return_type != FVAL_NUMBER) n = 0;

    if (return_type != FVAL_STRING) s.clear();

    if (return_type != FVAL_LIST) l.clear();
}
    ...
    if (method == "FUNCTIONS") {
        ...
        for (int i = 0; i < 2; i++) {
            if (envVect[3 * offset + i] != NULL) {
                ...
                cerr << "FileValue::Stochos - NULL envelope. Trying to access "
                        "envy["
        ...
        cerr << "FileValue::Stochos --- invalid method!  Use FUNCTIONS or "
                "RANGE_DISTRIB"
             << endl;
        exit(1);
    }
...
void FileValue::ftn_Select() {
    ...
    if (chooseNum >= choiceList->size() || chooseNum < 0) {
        cerr << "FileValue -- Error in Select(): index out of bounds of select "
                "list"
             << endl;
        cerr << "                    index = " << chooseNum << ", list has " << choiceList->size()
             << " items." << endl;
        if (evptr != NULL) {
            cerr << "                in file " << evptr->getEventName() << endl;
        }
...
void FileValue::ftn_ValuePick() {
    ...
    if (ftnArgs.size() == 10) {  // new format with offset
        args = EvaluateArgs(10, FVAL_NUMBER, FVAL_ENVELOPE, FVAL_ENVELOPE, FVAL_ENVELOPE,
                            FVAL_STRING, FVAL_LIST, FVAL_STRING, FVAL_LIST, FVAL_STRING, FVAL_LIST);
    } else {  // old format without offset
        args = EvaluateArgs(9, FVAL_NUMBER, FVAL_ENVELOPE, FVAL_ENVELOPE, FVAL_ENVELOPE,
                            FVAL_STRING, FVAL_LIST, FVAL_STRING, FVAL_LIST, FVAL_STRING);
    }
    ...
    // int offset =(ftnArgs.size()==10)? args[9]->getInt(): 0;  //default offset
    // = 0 if the function is in the old format
    ...
    // cout << "FileValue: ValuePick - minVal=" << minVal << " maxVal=" <<
    // maxVal << 	  " absRange=" << absRange << endl;
    Sieve si;
...
void FileValue::ftn_ExpandPattern() {
    ...
    vector<FileValue*> args =
        EvaluateArgs(5, FVAL_STRING, FVAL_NUMBER, FVAL_NUMBER, FVAL_NUMBER, FVAL_PATTERN);
    string method = args[0]->getString();
...
void FileValue::ftn_MakeEnvelope() {
    ...
    vector<FileValue*> args =
        EvaluateArgs(5, FVAL_LIST, FVAL_LIST, FVAL_LIST, FVAL_LIST, FVAL_NUMBER);
    ...
    if (xPoints->size() != yPoints->size() || interpTypes->size() != flexOrFix->size()) {
...
void FileValue::ftn_MakeSieve() {
    ...
    if (ftnArgs.size() == 7) {  // new format with offset
        args = EvaluateArgs(7, FVAL_NUMBER, FVAL_NUMBER, FVAL_STRING, FVAL_LIST, FVAL_STRING,
                            FVAL_LIST, FVAL_LIST);
    } else {  // old format
        args = EvaluateArgs(6, FVAL_NUMBER, FVAL_NUMBER, FVAL_STRING, FVAL_LIST, FVAL_STRING,
                            FVAL_LIST);
    }
    ...
    // int offset = (ftnArgs.size()==7)? args[6]->getInt(): 0; // default offset
    // = 0 if it's old format
...
void FileValue::static_ftn_DURATION_UNITS() {  // Deprecated--use AVAILABLE_EDU
    if (evptr == NULL) {
        cerr << "FileValue Error: DURATION_UNITS "  // Deprecated--use
                                                    // AVAILABLE_EDU
...
void FileValue::static_ftn_DURATION_EDU() {  // Deprecated--use AVAILABLE_EDU
    if (evptr == NULL) {
        cerr << "FileValue Error: DURATION_EDU "  // Deprecated--use
                                                  // AVAILABLE_EDU
             << "used outside of Event context" << endl;
        exit(1);
    }
...
void FileValue::static_ftn_AVAILABLE_EDU() {
    if (evptr == NULL) {
        cerr << "FileValue Error: AVAILABLE_EDU used outside of Event context" << endl;
        exit(1);
    }
    return_type = FVAL_NUMBER;
    n = evptr->getAvailableEDU();
...
void FileValue::static_ftn_CURRENT_LAYER() {
    if (evptr == NULL) {
        cerr << "FileValue Error: CURRENT_LAYER used outside of Event context" << endl;
        exit(1);
    }
    return_type = FVAL_NUMBER;
...
void FileValue::static_ftn_PREVIOUS_CHILD_DURATION() {
    if (evptr == NULL) {
        cerr << "FileValue Error: PREVIOUS_CHILD_DURATION used outside of "
                "Event context"
             << endl;
        exit(1);
    }
