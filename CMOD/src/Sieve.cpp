...
void Sieve::Elements(int minVal, int maxVal,
  ...
  if(strcmp(method, "MEANINGFUL") == 0) {		//only meaningful elem.
    ...
    Sieve::Fibonacci(minVal, maxVal, eArgVect[0]);
...
void Sieve::Fibonacci(int minVal, int maxVal, int offset) {
  skip = 0;
  eList.clear();
  int i = 0, j = 1;
  while (offset + i <= maxVal) {
    if (i != j && offset + i >= minVal) {
      eList.push_back(offset + i);
    }
    int k = i + j;
    i = j;
    j = k;
