...
string Sieve::getFileName() { return fileName; }

//---------------------------------------------------------------------------//

void Sieve::BuildFromExpr(int minVal, int maxVal, const char *eMethod, const char *wMethod,
...
void Sieve::Build(int minVal, int maxVal, const char *eMethod, const char *wMethod,
...
void Sieve::Elements(int minVal, int maxVal, const char *method, vector<int> eArgVect,
    ...
    if (strcmp(method, "MEANINGFUL") == 0) {  // only meaningful elem.
        ...
        cerr << "utility::SieveElements - multiple params not available yet" << endl;
        exit(1);
    } else {
...
void Sieve::Weights(std::vector<int> eArgVect, const char *method, std::vector<int> wArgVect,
                    std::vector<int> offsetVect) {
...
void Sieve::Meaningful(int minVal, int maxVal, vector<int> eArgVect, std::vector<int> offsetVect) {
    ...
    for (int i = 0; i < eArgVect.size(); i++) {
        if ((eArgVect[i] + offsetVect[i]) >= minVal && (eArgVect[i] + offsetVect[i]) <= maxVal) {
            /*
                cout << "Sieve::Meaningful - eArgVect[" << i << "]=" <<
               eArgVect[i]
...
void Sieve::Multiples(int minVal, int maxVal, vector<int> numMods, std::vector<int> offsetVect) {
   ...
   */
    for (int i = 0; i < numMods.size(); i++) {
        if (minVal == 0 && offsetVect[i] >= 0) {  // put 0 in the list if minVal ==0 and at
                                                  // least one of the offset is 0
        ...
        while (newElement <= maxVal) {
            if (newElement >= minVal) {
                // note: we don't have to check for duplicates before adding,
                // because
...
void Sieve::HierarchicWeights(const std::vector<int> &eArgVect, std::vector<int> wArgVect,
                              std::vector<int> offsetVect) {
    int whichMod;
    double probability;
