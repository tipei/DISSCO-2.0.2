 ...
 * \param return_type Enum holding the type to return (after computing any
 *functions) \param ftnString String indicating a name of a function \param
 *ftnArgs List of arguments passed to a function \param s String to be returned
 * \param n Number to be returned
 * \param l List of FileValue objects to be returned
 * \param obj Variable of type void* for holding different types of return
 *objects
 **/
class FileValue {
private:
...
public:
    ...
    //----------------//
    /**
     *  Method to return value of the original type that was parsed from the
     *input file \return Value of original type
     **/
     ...
     **/
    const string& getString(Event* ev) {
        Evaluate(ev);
        return s;
    }
     ...
     **/
    const int getInt(Event* ev) {
        Evaluate(ev);
        return (int)(getFtnString() == "GetPattern"
                         ? ((Patter*)obj)->GetNextValue(patterMethod, patterOrigin)
                         : n);
    }
     ...
     *  Evaluate any functions and return an int
     **/
    const float getFloat(Event* ev) {
        Evaluate(ev);
        return (float)(getFtnString() == "GetPattern"
                           ? ((Patter*)obj)->GetNextValue(patterMethod, patterOrigin)
                           : n);
    }
     ...
     **/
    const list<FileValue>& getList(Event* ev) {
        Evaluate(ev);
        return l;
    }
     ...
     **/
    list<FileValue>* getListPtr(Event* ev) {
        Evaluate(ev);
        return &l;
    }
