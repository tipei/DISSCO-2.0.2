...
Matrix::Matrix(int numTypes, int numAttacks, int numDurations, vector<int> numTypesInLayers,
               int maxVal, Tempo tempo, bool sieveAligned)
    : sieveAligned(sieveAligned), tempo(tempo) {
    matr.clear();
    ...
    for (int type = 0; type < matr.size(); type++) {
        ...
        for (int attNum = 0; attNum < matr[type].size(); attNum++) {
            ...
            for (int durNum = 0; durNum < matr[type][attNum].size(); durNum++) {
                ...
                /*
                        cout << "Matrix::setAttacks - matr[" << type << "][" <<
                   attNum << "]["
                             << durNum << "].attdurprob=" <<
                   matr[type][attNum][durNum].attdurprob
                             << endl;
                */
            }
        }
        ...
        for (int attNum = 0; attNum < matr[type].size(); attNum++) {
            ...
            for (int durNum = 0; durNum < matr[type][attNum].size(); durNum++) {
                ...
                if (durNum < 0 || durNum > matr[type][attNum].size()) {
                    cerr << "Matrix::setDurations -- error - getValue out of "
                            "bounds;"
                         << endl;
                    cerr << "  durNum=" << durNum
                         << ", matr[type][attNum].size()=" << matr[type][attNum].size() << endl;
                }
                /*
                        if(type != oldType) {
                          cout << "Matrix::setDurations - envelope for type " <<
                   type << endl; durEnvs[type] -> print(); int sever; cin >>
                   sever; oldType = type;
                        }
                */
                ...
                /*
                        cout << endl;
                        cout << "durEnd=" << durEnd << " maxVal=" << maxVal <<
                   endl; cout << "  matr[" << type << "][" << attNum
                             << "][" << durNum << "].stime=" <<
                   matr[type][attNum][durNum].stime
                             << " matr[" << type << "][" << attNum << "][" <<
                   durNum
                             << "].attdurprob=" <<
                   matr[type][attNum][durNum].attdurprob << endl;
                */
                if ((sieveAligned && att.count(durEnd % tempo.getEDUPerTempoBeat().Num()) == 0 &&
                     att.count(durEnd) == 0) ||
                    durEnd > maxVal) {
                    matr[type][attNum][durNum].attdurprob = 0;
                    /*
                              cout << "sieveAligned - type=" << type << "
                       attNum=" << attNum
                                   << " durNum=" << durNum << " maxVal=" <<
                       maxVal << endl; int sever; cin >> sever;
                    */
                } else {
                    matr[type][attNum][durNum].attdurprob *= durSieveVal;
                    if (hasEnv) {
                        double durEnvVal =
                            durEnvs[type]->getValue(durNum, matr[type][attNum].size());

                        //      cout << "Matrix::setDurations - durEnvVal=" <<
                        //      durEnvVal << endl;
                        //	int sever; cin >> sever;
                ...
                /*
                          cout << "  stime=" << matr[type][attNum][durNum].stime
                   << " dur=" << matr[type][attNum][durNum].dur << endl; cout <<
                   "    durSieveVal=" << durSieveVal
                           << " matr[" << type << "][" << attNum << "][" <<
                   durNum
                           << "].attdurprob=" <<
                   matr[type][attNum][durNum].attdurprob << endl;
...
//---------------------------------------------------------------------------//
void Matrix::setTypeProbs(vector<double> typeProbVect) { typeProb = typeProbVect; }
...
MatPoint Matrix::chooseContinuum() { return choose(); }
        ...
        for (int attNum = 0; !found && attNum < matr[type].size(); attNum++) {
            ...
            for (int durNum = 0; !found && durNum < matr[type][attNum].size(); durNum++) {
                ...
                if (matr[type][attNum][durNum].normprob >= randNum) {
                    ...
                    // printing stuff
                    /*
                              cout << endl << "MATRIX ---- PICKED: matr[" <<
                    type << "]["
                                  << attNum << "][" << durNum << "]  =  start:"
                                  << chosenPt.stime << ", dur:" << chosenPt.dur
                    << ", type:"
                                  << chosenPt.type << ", randNum=" << randNum <<
                    endl; cout << "  chosenPt.normprob=" << chosenPt.normprob <<
                    ", prevPt.normprob="
                                   << prevPt.normprob << ", %prob:"
                                   << (chosenPt.normprob-prevPt.normprob) <<
                    endl; cout << "  " << endl;
                    //        printMatrix(true);
                              int sever; cin >> sever;
                    */
        ...
        for (int attNum = 0; attNum < matr[type].size(); attNum++) {
            ...
            for (int durNum = 0; durNum < matr[type][attNum].size(); durNum++) {
                ...
                /*
                 ...
                 attackSieve"][" << durNum
                                << "].normprob= " <<
                 matr[type][attNum][durNum].normprob << endl; cout << "
                 matrSum=" << matrSum << endl;
                        }
                */
            }
        }
...
int Matrix::verify_valid(int endTime) {
    ...
    // cout << "endTime: " << eTime << " high_n: " << valid_time[high] << "
    // low_n: " << valid_time[low] <<  endl;
    int offset = short_attime[high] - eTime <= eTime - short_attime[low]
                     ? short_attime[high] - eTime
                     : short_attime[low] - eTime;
    // cout << "endTime: " << endTime << " choose: " << endTime + offset <<
    // endl;
...
void Matrix::removeConflicts(MatPoint &chosenPt) {
    ...
    // cout << "removeConflicts : start: " << chosenStart << " end: " <<
    // chosenEnd <<  endl;
}
...
void Matrix::removeSweepConflicts(MatPoint &chosenPt) {
    ...
    // cout << "removeSweepConflicts : start: " << chosenStart << " end: " <<
    // chosenEnd <<  endl;
}
