...
cout << "begin printing vector: " << endl;
	...
	Note * c = *(soundsAndRests.begin());

	for (it2 = soundsAndRests.begin(); it2 != soundsAndRests.end(); it2++) {
	  c = *it2;
	  cout << "c: " << (c->end_t - c->start_t) << " " << c->pitch_out << endl;
...
void Note::createRest(int stime, int endTime, string loud){
  Note * t = new Note();
  t->start_t = stime;
  t->end_t = endTime;
  t->pitch_out = "r";
...
void Note::checkDuration(int & stime, int & endTime) {
  ...
  int flag = 0;
  if (checkValue(dur) == 1) return;
cout << "checkedValue" << endl;
...
int Note::closestVal(int remain, int dur, int beatNum) {
  ...
  for (int i = 1; i<13; i++) {
    ...
    if (flag == 0 && tempEnd <= beatEDUs) {

cout << "   closestVa l- flag=" << flag << " remain=" << remain << " dur="
     << dur << " tempEnd=" << tempEnd << " close=" << close << " keep="
     << keep << endl;

      if (abs(valid_time[i] - dur) < close) {
        close = abs(valid_time[i] - dur);
        keep = tempEnd;
      }
    }
  }
