...
Bottom::Bottom(DOMElement* _element, TimeSpan _timeSpan, int _type, Tempo _tempo,
               Utilities* _utilities, DOMElement* _ancestorSpa, DOMElement* _ancestorRev,
...
void Bottom::buildChildren() {
    ...
    for (currChildNum = 0; currChildNum < numChildren; currChildNum++) {
        if (method == "0")  // continuum
            ...
            //      cout<<endl;  // Ming-ching: I don't know why the program
            //      crash without this line... very odd..
    ...
    // Using the temporary events that were created, construct the actual
    // children. The code below is different from buildchildren in Event class.
...
void Bottom::modifyChildren() {  // Incomplete Override
    ...
    // cout<<*loudnessElement->getNodeValue(); //Checking if update is
    // successful
}
...
void Bottom::buildSound(SoundAndNoteWrapper* _soundNoteWrapper) {
    ...
    if (utilities->getOutputParticel()) {
        ...
        Output::addProperty("End Time",
                            _soundNoteWrapper->ts.start + _soundNoteWrapper->ts.duration, "sec.");
    ...
    DOMElement* specElement = _soundNoteWrapper->element->GFEC()->GNES()->GNES()->GNES()->GNES();

    // if there is no spec element,go to partial routine;else, go to
    // gen_spectrum
    if (specElement->GFEC() != NULL) {
        DOMElement* partialEnvElement =
            (DOMElement*)utilities->evaluateObject(XMLTC(specElement), (void*)this, eventSpec);
        DOMElement* distanceElement = partialEnvElement->GNES();

        Envelope* waveShape =
            (Envelope*)utilities->evaluateObject(XMLTC(partialEnvElement), (void*)this, eventEnv);
        float distance = utilities->evaluate(XMLTC(distanceElement), (void*)this);

        // instead of reading partials, generate spectrum envelope and add to
        // sound
        ...
        if (utilities->getOutputParticel())
            Output::addProperty("Deviation", computeDeviation(_soundNoteWrapper->element),
                                "normalized");
        ...
        for (int i = 0; i < numPartials; i++) {
            ...
            if (i != 0) deviation = computeDeviation(_soundNoteWrapper->element);

            // Set the frequencies for each partial.
            float actualFrequency = setPartialFreq(partial, deviation, baseFrequency, i);
...
void Bottom::buildNote(SoundAndNoteWrapper* _soundNoteWrapper) {
    // Create the note.
    // Note* newNote = new Note();		//sever, make similar to
    // newSound
    ...
    if (utilities->getOutputParticel()) {
        ...
        Output::addProperty("End Time",
                            _soundNoteWrapper->ts.start + _soundNoteWrapper->ts.duration, "sec.");
        Output::addProperty("Duration", _soundNoteWrapper->ts.duration, "sec.");
        Output::addProperty("Tempo Start Time", _soundNoteWrapper->tempo.getStartTime(), "sec.");
        Output::addProperty("EDU Start Time", _soundNoteWrapper->ts.startEDU.toPrettyString(),
                            "EDU");
        Output::addProperty("EDU Duration", _soundNoteWrapper->ts.durationEDU.toPrettyString(),
                            "EDU");
    }
    ...
    Output::endSubLevel();
    // cout << "note's data: " << newNote -> pitch_out << " " << newNote ->
    // start_t << " " << newNote -> end_t << endl;
...
list<Note> Bottom::getNotes() {
    list<Note> result;
    for (int i = 0; i < childNotes.size(); i++) result.push_back(*childNotes[i]);
    return result;
}
...
int Bottom::computeNumPartials(float baseFreq, DOMElement* _spectrum) {
    DOMElement* numPartialElement = _spectrum->GFEC()->GNES()->GNES();
    ...
    if (numPartsResult <= 0) {
        cerr << "Error: Bottom::computeNumPartials got 0, baseFrequency=" << baseFreq << endl;
        exit(1);
    }
...
float Bottom::computeDeviation(DOMElement* _spectrum) {
    DOMElement* devElement = _spectrum->GFEC()->GNES()->GNES()->GNES();
...
float Bottom::setPartialFreq(Partial& part, float deviation, float baseFreq, int partNum) {
    ...
    // if pFreq is out of range then set it to the closer of the max or min
    // value
    if (pFreq < MINFREQ) {
        pFreq = MINFREQ;
...
void Bottom::setPartialSpectrum(Partial& part, int partNum, DOMElement* _element) {
    DOMElement* partialEnvElement =
        _element->GFEC()->GNES()->GNES()->GNES()->GNES()->GNES()->GFEC();
...
void Bottom::applySpatialization(Sound* s, int numPartials) {
    DOMElement* SPAElement = (DOMElement*)utilities->evaluateObject(
        "", (void*)this,
        eventSpa);  // this call will return a spa function, just in case users
                    // use "select" here. The string here is just a dummy since
                    // the callee will find the right spa element  within "this".
    ...
    if (method.compare("STEREO") == 0) {
        ...
        cout << "SOUND_SPATIALIZATION has invalid method!  Use STEREO, "
                "MULTI_PAN, or POLAR"
             << endl;
        exit(1);
    }
...
void Bottom::spatializationStereo(Sound* s, DOMElement* _channels, string applyHow, int numParts) {
    //  <Channels>
    //    <Partials>
    ...
    Envelope* panning =
        (Envelope*)utilities->evaluateObject(XMLTC(envelopeElement), (void*)this, eventEnv);
...
void Bottom::spatializationMultiPan(Sound* s, DOMElement* _channels, string applyHow,
                                    int numParts) {
    //  <Channels>
    //    <Partials>
...
void Bottom::spatializationPolar(Sound* s, DOMElement* _channels, string applyHow, int numParts) {
    //<Channels>
...
void Bottom::applyFilter(Sound* s) {
     ...
     **/
    /**
     *	BiQuadFilter(int type, m_sample_type dbGain,  m_sample_type freq,
     *m_sample_type srate,   m_sample_type bandwidth); Where arguments are
     * 1)Filter type: 0-6:
     *0-Low Pass Filter,
     *1-High Pass Filter
     *2-Band Pass Filter
    ...
    BiQuadFilter* filterObj =
        new BiQuadFilter(typeInt, gain, frequency, utilities->getSamplingRate(), bandWidth);
...
void Bottom::applyReverberation(Sound* s) {
    DOMElement* reverbElement = (DOMElement*)utilities->evaluateObject("", (void*)this, eventRev);

    // this call will return a rev function, just in case users use "select"
    // here. The string here is just a dummy since the callee will find the right
    // rev element  within "this".
    ...
    if (rev_method.compare("REV_Simple") == 0) {
        ...
        Reverb* reverbObj =
            new Reverb(percent_rev, hi_low_spread, gain_all_pass, delay, SAMPLING_RATE);
        s->use_reverb(reverbObj);
        delete percent_rev;
        ...
        //	    <Fun><Name>EnvLib</Name><Env>1</Env><Scale>1.0</Scale></Fun>
        //	  </Percent>
        //	  <CombGainList>0.46, 0.48, 0.50, 0.52, 0.53,
        // 0.55</CombGainList> 	  <LPGainList>0.05, 0.06, 0.07, 0.05, 0.04,
        // 0.02</LPGainList> 	  <AllPass></AllPass> 	  <Delay></Delay>
        //	</Fun>
        DOMElement* argument = reverbElement->GFEC()->GNES();
        ...
        vector<std::string> stringListC = utilities->listElementToStringVector(argument);
        ...
        vector<std::string> stringListG = utilities->listElementToStringVector(argument);
        ...
        Reverb* reverbObj = new Reverb(percent_rev, &comb_gain_list[0], &lp_gain_list[0],
...
void Bottom::applyModifiers(Sound* s, int numPartials) {
    ...
    // cout<<"Bottom-"<<name<<": Modifiers after
    // merge:"<<XMLTC(modifiersIncludingAncestorsElement)<<endl<<endl<<"============="<<endl;
...
vector<string> Bottom::applyNoteModifiersOld() {
    ...
    cout << "Bottom-" << name
         << ": Modifiers after merge:" << XMLTC(modifiersIncludingAncestorsElement) << endl
         << endl
         << "=============" << endl;
    ...
    while (modifierElement != NULL) {
        ...
        /* needs to be rewrite to remove filevalue
          while (modIter != modList->end()) {
            ...
            list<FileValue>::iterator currMod =
          modIter->getListPtr(this)->begin();
        ...
        */
        ...
        /* needs to be rewrite to remove filevalue
            ...
            while (mutExGroup == "" && currMod !=
           modIter->getListPtr(this)->end()) { FileValue mutExFV =
           currMod->getListPtr(this)->back(); if (mutExFV.getReturnType() ==
           FVAL_STRING) { mutExGroup = mutExFV.getString(this); } else { cerr <<
           "Bottom::applyModifiers error: invalid syntax for MUT_EX group!" <<
           endl; exit(1);
              }
        */
...
void Bottom::generatePartials(Sound* newsound, float frequency, float loudness, float distance,
                              Envelope* waveShape) {
    ...
    double scaleTable[3][3][20] = {
        {// Bb3,p,m,f     233 hz
         {0.886, 0.014, 1,     0.072, 0.367, 0.193, 0.183, 0.036, 0.058, 0.010,
          0.028, 0.021, 0.011, 0.016, 0.003, 0.003, 0.002, 0.002, 0.002, 0.002},
         {0.756, 0.014, 1,     0.037, 0.521, 0.217, 0.234, 0.123, 0.306, 0.066,
          0.142, 0.040, 0.053, 0.044, 0.053, 0.039, 0.033, 0.043, 0.004, 0.012},
        ...
        {// Bb5,p,m,f     932 hz
         ...
         {1.00,  0.507, 0.132, 0.160, 0.030,  0.002,  0.001, 0.0008, 0.0016, 0.003,
          0.002, 0.001, 0.001, 0.001, 0.0005, 0.0005, 0.001, 0.0005, 0.001,  0.00064},
         {1.00,  0.354, 0.198, 0.102, 0.054, 0.012,  0.005, 0.002, 0.005, 0.003,
          0.003, 0.002, 0.001, 0.002, 0.004, 0.0015, 0.002, 0.002, 0.003, 0.0013}}};
    // Set the number of partials.
    int numPartials = 20;
    ...
    for (int i = 0; i < numPartials; i++) {
        ...
        // Set the frequencies for each partial.
        float actualFrequency = setPartialFreq(partial, 0, frequency, i);
        ...
        if (frequency <= 466) {
            if (strength <= 1) {
                double scale_strength1 = calculateFreqPartial(233, scaleTable[0][0][i], 466,
                                                              scaleTable[1][0][i], frequency);
                double scale_strength2 = calculateFreqPartial(233, scaleTable[0][1][i], 466,
                                                              scaleTable[1][1][i], frequency);
                scale = strength * scale_strength1 + (1 - strength) * scale_strength2;
            } else {
                double scale_strength1 = calculateFreqPartial(233, scaleTable[0][1][i], 466,
                                                              scaleTable[1][1][i], frequency);
                double scale_strength2 = calculateFreqPartial(233, scaleTable[0][2][i], 466,
                                                              scaleTable[1][2][i], frequency);
                scale = (2 - strength) * scale_strength1 + (strength - 1) * scale_strength2;
            }
        } else {
            if (strength <= 1) {
                double scale_strength1 = calculateFreqPartial(466, scaleTable[1][0][i], 932,
                                                              scaleTable[2][0][i], frequency);
                double scale_strength2 = calculateFreqPartial(466, scaleTable[1][1][i], 932,
                                                              scaleTable[2][1][i], frequency);
                scale = strength * scale_strength1 + (1 - strength) * scale_strength2;
            } else {
                double scale_strength1 = calculateFreqPartial(466, scaleTable[1][1][i], 932,
                                                              scaleTable[2][1][i], frequency);
                double scale_strength2 = calculateFreqPartial(466, scaleTable[1][2][i], 932,
                                                              scaleTable[2][2][i], frequency);
                scale = (2 - strength) * scale_strength1 + (strength - 1) * scale_strength2;
            }
        }
...
double Bottom::calculateFreqPartial(double x1, double y1, double x2, double y2, double x) {
    ...
    // the curve between two points (x1,y1) and (x2,y2) is modeled by
    // y=a*2^(b*(x-x1)) assuming y2>y1
    if ((x > x2) || (x < x1)) {
    ...
    if (y2 >= y1) {
        ...
        // horizontal line y1) to the above case. Model the curve as if y2>y1
        // and take the symmetric result
