...
void Modifier::addValueEnv(Envelope* env) { env_values.push_back(new Envelope(*env)); }
...
string Modifier::getModName() { return type; }
...
void Modifier::applyModSound(Sound* snd) {
    if (type == "FREQUENCY" || type == "GLISSANDO" || type == "DETUNE" || type == "BEND") {
