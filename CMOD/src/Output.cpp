...
OutputNode::~OutputNode() {
    for (int i = 0; i < (int)subNodes.size(); i++) delete subNodes[i];
    propertyNames.clear();
...
bool OutputNode::isBottom(void) { return (nodeName[0] == 'B' && nodeName[1] == '/'); }

//----------------------------------------------------------------------------//

bool OutputNode::isNote(void) { return (nodeName == "Note"); }
...
string OutputNode::getProperty(string name) {
    for (unsigned int i = 0; i < propertyNames.size(); i++)
        if (sanitize(propertyNames[i]) == sanitize(name)) return propertyValues[i];
    return "";
}
...
string OutputNode::getXML(void) {
    ...
    for (int i = 0; i < subNodes.size(); i++) {
        if (subNodes[i]->isBuildPhase()) continue;
        s += subNodes[i]->getXML();
    }
...
//----------------------------------------------------------------------------//
// move data in subNodes to fomusdata, each bottom event corresponding to one
...
string OutputNode::findAndReplace(string in, string needle, string replace) {
    while (in.find(needle) != string::npos) in.replace(in.find(needle), needle.length(), replace);
    return in;
}
...
string Output::getPropertyIndentation(void) { return getLevelIndentation(true, false) + ". "; }
...
OutputNode* Output::getCurrentLevelNode(void) {
    ...
    for (int i = 1; i <= level; i++) currentNode = currentNode->subNodes.back();
    return currentNode;
}
...
void Output::beginSubLevel(string name) {
    ...
    writeLineToParticel(getLevelIndentation(false, false) + getCurrentLevelNode()->nodeName);
}
...
void Output::addProperty(string name, string value, string units) {
    ...
    if (!current)
        cerr << "Warning: Top level does not exist. Property can not be added." << endl;
    else
    ...
    if (units != "") stringToWrite = stringToWrite + " " + units;
