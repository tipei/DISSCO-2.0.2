...
Event::Event(DOMElement* _element, TimeSpan _timeSpan, int _type, Tempo _tempo,
    ...
    if (tempo.getStartTime() == 0) {
        ...
        cout << endl
             << "WARNING: the tempo of this exact event differs from" << endl
    ...
    if (XMLTC(flagElement) == "0") {  // Continuum
        ...
        //  cout<<"density:"<< density<<", area:"<<area<<",
        //  underOne:"<<underOne<<endl;
    ...
    */
    ...
    for (int i = 0; i < childEvents.size(); i++) {
        ...
        */
        ...
        // TODO: DELETE ELSEWHERE. Cleanup should probably be done after
        // execution.
...
void Event::findLeafChildren(vector<Event*>& leafChildren) {
    if (childEvents.size() == 0) {
        ...
        cout << "numchildren: " << childEvents.size() << "----Non-Leaf----: " << name << endl;
    }
    for (int i = 0; i < childEvents.size(); i++) {
        cout << "Before a dereference in findLeafChildren in " << name << endl;
        if (childEvents[i] != NULL) childEvents[i]->findLeafChildren(leafChildren);
...
bool Event::buildContinuum() {
    ...
    bool align =
        (sieveAligned && Utilities::isSieveFunction(childStartTimeElement) &&
         Utilities::isSieveFunction(childDurationElement) && startType == "1" && durType == "1");
    ...
    if (align) {
        ...
        if (startType == "1") {  //"EDU"
            tsChild.start = rawChildStartTime * tempo.getEDUDurationInSeconds().To<float>();
        ...
        if (rawChildDurationInt > maxChildDurInt) rawChildDurationInt = maxChildDurInt;
        ...
        if (durType == "1") {  // EDU
            ...
            if (tsChild.duration > maxChildDur) tsChild.duration = maxChildDur;  // enforce limit
...
bool Event::buildSweep() {
    ...
    if (checkPoint > 1) {
        ...
        cerr << "      childStime=" << tsChild.start << ", parentDur=" << ts.duration << endl;
        cerr << "      in file: " << name << ", childNum=" << currChildNum << endl;
    ...
    if (align) {
        ...
        if (startType == "1") {  // EDU
            tsChild.start =
                rawChildStartTime * tempo.getEDUDurationInSeconds().To<float>() + tsPrevious.start;
            ...
            tsChild.start =
                rawChildStartTime * ts.duration + tsPrevious.start;  // convert to seconds
            tsChild.durationEDU = Ratio(0, 0);  // floating point is not exact: NaN
        }
...
void Event::tryToRestart(void) {
    ...
    if (restartsRemaining > 0) {
        restartsRemaining--;
        cout << "Failed to build child " << currChildNum << " of " << numChildren << " in file "
             << name << ". There are " << restartsRemaining << " tries remaining." << endl;
        //  int sever; cin >> sever;
    } else {
        ...
        while (!inputAccepted) {
            discreteFailedResponse = "";
            pthread_create(&discreteWaitForInputIfFailedThread, NULL, waitForDiscreteResponse,
                           (void*)this);
            void* myfail;
            ...
            // warning! memory leak here! There is a problem killing thread
            // waiting for cin. need to figure this out.
    ...
    // NOTE: SHOULD BE -1
    currChildNum = -1;
    for (int i = 0; i < childEvents.size(); i++) delete temporaryChildEvents[i];
    temporaryChildEvents.clear();
...
void Event::checkEvent(bool buildResult) {
    ...
    yet within the piece. The following section uses the start time/tempo rules
    to determine the correct exact and inexact start times, in some cases
    leading to a new tempo.*/

    /*Inexact start time is global. That is, it *always* refers to the position
    in time relative to the beginning of the piece. Thus the child start time is
    ...
    Inexact Start Times (~ means the exact value is truncated to floating
    point): Event 1 = i1 Event 2 = i1 + ~e2 Event 3 = i1 + ~e2 + ~e3 Event 4 =
    i1 + ~e2 + ~e3 + i4 Event 5 = i1 + ~e2 + ~e3 + i4 + i5 Event 6 = i1 + ~e2 +
    ~e3 + i4 + i5 + ~e6
    ...
    Important Note:
    If the child attempts to override the parent tempo, it will be ignored and
    the above calculation. This is to prevent implicitly nested tempos, which
    are better handled explicitly at the moment. For example it would be very
    ...
    if (ts.startEDU.isDeterminate() && tsChild.startEDU.isDeterminate()) {
        ...
        /*We need to force child to have the same tempo, so that weird things do
        not happen. This is done below by explictly setting the tempo of the
        child. This will in turn be honored by initDiscreteInfo which will not
        override the given parent tempo. Note in order for this to be done, the
        tempo is passed to buildChildEvents, to constructChild, to
        EventFactory::Build, and finally to initDiscreteInfo.*/
    }
    ...
    offset an exact amount from the parent, the parent is the new tempo
    reference.

    This could easily be the source of confusion: when a parent offset is
    inexact, and a child offset is exact, it is the parent which takes on the
    new tempo. Note that this implies that the child's siblings will refer to
    the same new tempo start time.*/
    if (!ts.startEDU.isDeterminate() && tsChild.startEDU.isDeterminate()) {
        ...
        parent. If this is the second exact child of a parent, then it will
        merely set the start time to the same thing.*/
    ...
    EventType childEventType =
        (EventType)utilities->evaluate(XMLTC(discretePackage->GFEC()->GNES()), (void*)this);
    ...
    if (childEventType == eventBottom) {
        ...
        childSoundsAndNotes.push_back(
            new SoundAndNoteWrapper(childElement, tsChild, childEventName, childType, tempo));
    }
    /*
      else if (childEventType == eventNote) {
        childSoundsAndNotes.push_back(new SoundAndNoteWrapper
                                        (childElement,tsChild, childType,
      tempo));
      }
    */
    else {
        e = new Event(childElement, tsChild, childType, tempo, utilities, spatializationElement,
...
void Event::outputProperties() {
    ...
    Output::addProperty("Tempo", tempo.getTempoBeatsPerMinute().toPrettyString(), "BPM");
    ...
    Output::addProperty("Divisions", tempo.getEDUPerTimeSignatureBeat().toPrettyString(),
                        "EDU/beat");
...
int Event::getCurrentLayer() {
    ...
    for (int i = 0; i < layerVect.size(); i++) {
        ...
        if (childType < countInLayer) return i;
    }
    cerr << "Unable to get layer number in file " << name << endl;
    exit(1);
}
...
int Event::getAvailableEDU() {
    ...
    if (ts.startEDU.isDeterminate() && ts.startEDU != Ratio(0, 1)) return ts.startEDU.To<int>();
    ...
    if (ts.duration == (float)myDurationInt) {
        // Since duration is an integer, it may still be possible to have exact
        // EDUs.
...
bool Event::buildDiscrete() {
    ...
    if (durEDU > (int)maxChildDur) durEDU = maxChildDur;
    tsChild.startEDU = stimeEDU;
    tsChild.durationEDU = durEDU;

    tsChild.start = (float)stimeEDU * tempo.getEDUDurationInSeconds().To<float>();
    tsChild.duration = (float)durEDU * tempo.getEDUDurationInSeconds().To<float>();
...
void Event::buildMatrix(bool discrete) {
    ...
    if (discrete) {
        attackSiv =
            (Sieve*)utilities->evaluateObject(XMLTC(AttackSieveElement), (void*)this, eventSiv);

        durSiv =
            (Sieve*)utilities->evaluateObject(XMLTC(DurationSieveElement), (void*)this, eventSiv);
    } else {
    ...
    for (int i = 0; i < childTypeElements.size(); i++) {
        double prob =
            utilities->evaluate(XMLTC(childTypeElements[i]->GFEC()->GNES()->GNES()), (void*)this);
    ...
    if (discrete) {
        for (int i = 0; i < childTypeElements.size(); i++) {
            ...
            string attackFunctionString = "<Fun><Name>EnvLib</Name><Env>" + attackEnvString +
                                          "</Env><Scale>" + attackEnvScaleString + "</Scale></Fun>";
            ...
            string durationFunctionString = "<Fun><Name>EnvLib</Name><Env>" + durationEnvString +
                                            "</Env><Scale>" + durationEnvScaleString +
                                            "</Scale></Fun>";

            attackEnvs.push_back(
                (Envelope*)utilities->evaluateObject(attackFunctionString, this, eventEnv));

            durEnvs.push_back(
                (Envelope*)utilities->evaluateObject(durationFunctionString, this, eventEnv));
        }
    }
    for (int i = 0; i < layerElements.size(); i++) {
    ...
    matrix = new Matrix(childTypeElements.size(), attackSiv->GetNumItems(), durSiv->GetNumItems(),
                        numTypesInLayers, parentEDUs, tempo, sieveAligned);
