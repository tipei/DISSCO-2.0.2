...
private:
  ...
  /**
   *  Fibonacci makes a list of elements which follows a Fibonacci sequence
   *  \param minVal, minimum value allowed
   *  \param maxVAl, maximum value allowed
   *  \param offset, the start point of the sequence
   *  For example, minVal=0, maxVal=(large) offset = 0 -> output=1,2,3,5,8,...
   *  For example, minVal=10, maxVal=(large) offset =10 -> output=11,12,13,15,18,23...
   *  For example, minVal=18, maxVal=(large) offset =10 -> output=18,23...
   */
  void Fibonacci(int minVal, int maxVal, int offset);
