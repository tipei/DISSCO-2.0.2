...
BaseFoo* foo = new SuperFoo(Arg1, Arg2) --> BaseFoo* foo = NewC((BaseFoo*)(new SuperFoo(Arg1, Arg2)))
*/

//Turn off internal leak checker for Windows, VLD, or when optimizations are on.
...
#else
#define New(x) (new x)
template<class T> T* NewC(T* x) {return x;}
template<class T> void Delete(T*& x) {delete x; x = 0;}
#endif
...
class MemoryLeak
{
  ...
  /**Reports memory leaks. False positives may be occurring if there are entries
  for more deletes than news. In this case, check to make sure NewC and Delete
  are being correctly used for subclass new-base class deletes.*/
  static void Report()
  {
  ...
  static T* DebugNew(int LineNumber, const char* Filename)
  {
    ...
    {
      std::cout << "New:    " << typeid(T).name() << " at " << Filename << ":" <<
        LineNumber << " = " << GetCode(0, typeid(T).name()) << std::endl;
  ...
  /**Delete that registers deleting a memory allocation by line number and file.
  Also sets the object pointer (passed by reference) to zero.*/
  template <class T>
  static void DebugDelete(T*& Object, int LineNumber, const char* Filename)
  {
    if(!Object)
      return; //Exclude null pointers from deletion tracking.
    delete Object;
    Object = 0;
    if(!Initialized)
      Initialize();
    ...
    {
      std::cout << "Delete: " << typeid(T).name() << " at " << Filename << ":" <<
        LineNumber << " = " << GetCode(0, typeid(T).name()) << std::endl;
