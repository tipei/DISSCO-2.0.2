...
//----------------------------------------------------------------------------//
Track *Partial::render(m_sample_count_type sampleCount, m_time_type duration,
                       m_rate_type samplingRate) {
    // calculate the number of samples needed:
    m_sample_count_type numSamplesToRender =
        (m_sample_count_type)(duration * (m_time_type)samplingRate);
    m_sample_count_type numSamplesTotal;
    if (reverbObj != NULL)
        numSamplesTotal =
            (m_sample_count_type)((duration + reverbObj->getDecay()) * (m_time_type)samplingRate);
    else
    ...
    //    SoundSample* waveSample = new SoundSample(numSamplesTotal,
    //    samplingRate);
    SoundSample *waveSample = new SoundSample(sampleCount, samplingRate);
    // and one to store amplitude data:
    //    SoundSample* ampSample = new SoundSample(numSamplesTotal,
    //    samplingRate);
    ...
    for (m_sample_count_type s = 0; s < numSamplesToRender; s++) {
        ...
        if (amptranscheck <= 0 && s + amptrans_width < numSamplesToRender) {
            ...
            if (random <= amptransprob) {
                ...
                if (((float)std::rand() / (float)RAND_MAX) <= 0.5) trans_amplifier *= -1;
        ...
        // augment the amplitude value by the tremolo value, and the loudness
        // scale:
        if (amptransflag) {
            ...
            amptransplace++;
            if (amptransplace >= amptrans_width) amptransflag = 0;
        } else
            amptransient = 0;
        ...
        if (freqtranscheck <= 0 && s + freqtrans_width < numSamplesToRender) {
            ...
            if (random <= freqtransprob) {
                ...
                if (((float)std::rand() / (float)RAND_MAX) <= 0.5) trans_freqmod *= -1;
        ...
        if (freqtransflag) {
            if (freqtransplace < freqtrans_width / 2)
                ...
                freqtransient =
                    freqmod * (freqtrans_width - freqtransplace) / (freqtrans_width / 2);
            freqtransplace++;
            if (freqtransplace >= freqtrans_width) freqtransflag = 0;
        } else
            freqtransient = 0;
        freqtranscheck--;
...
//----------------------------------------------------------------------------//
void Partial::use_reverb(Reverb *newReverbObj) { reverbObj = newReverbObj; }
...
//----------------------------------------------------------------------------//
void Partial::xml_print(ofstream &xmlOutput, list<Reverb *> &revObjs,
    ...
    // Static Parameters
    xmlOutput << "\t\t\t<relative_amplitude value=\"" << getParam(RELATIVE_AMPLITUDE) << "\" />"
              << endl;
...
void Partial::xml_read(XmlReader::xmltag *partialtag, DISSCO_HASHMAP<long, Reverb *> *reverbHash,
    ...
    if ((value = partialtag->findChildParamValue("relative_amplitude", "value")) != 0)
    ...
    // For sake of ease, I am going to forego searching for each item and
    // instead iterate thru the list of child tags.

    XmlReader::xmltag *dvtag;
    XmlReader::xmltagset *child = partialtag->children;
...
void Partial::auxLoadParam(enum PartialDynamicParam param, XmlReader::xmltag *tag,
                           DISSCO_HASHMAP<long, DynamicVariable *> *dvHash) {
    char *value;
    ...
    if ((value = tag->findChildParamValue("dv_ptr", "id")) != 0) {
        ...
        if (dv)
            ...
            // printf("[%s:%d] PartialParam refrences non-existant dv
            // %d!\n",__FILE__,__LINE__,id);
            cout << "[" << __FILE__ << ":" << __LINE__
                 << "] PartialParam references non-existant dv " << id << "!" << endl;
    }
