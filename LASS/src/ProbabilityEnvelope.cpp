...
m_value_type lerp(m_value_type a, m_value_type b, double c) { return (1 - c) * a + c * b; }

ProbabilityEnvelope::ProbabilityEnvelope() { setDuration(1.0); }
...
m_value_type ProbabilityEnvelope::sample(double x) {
    ...
    size_t upper_idx =
        std::upper_bound(cumulativeStepCounts.begin(), cumulativeStepCounts.end(), target) -
        cumulativeStepCounts.begin();

    // the value passed in is close to or over 1
    if (upper_idx == stepCounts.size()) return 1.0;

    // the value passed in is close to or under 0
    if (upper_idx == 0) return 0.0;
