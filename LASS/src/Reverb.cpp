...
Reverb::Reverb(m_rate_type samplingRate) {
    ...
    ConstructorCommon(percentReverb, &comb_gain_list[0], &lp_gain_list[0], 0.7, 0.6, samplingRate);
}
...
Reverb::Reverb(float room_size, m_rate_type samplingRate) {
    ...
    ConstructorCommon(percentReverb, &comb_gain_list[0], &lp_gain_list[0], gainAllPass, delay,
                      samplingRate);
}

Reverb::Reverb(Envelope *percentReverbinput, float hilow_spread, float gainAllPass, float delay,
...
Reverb::Reverb(Envelope *percentReverbinput, float *combGainList, float *lpGainList,
    ...
    ConstructorCommon(percentReverbinput, combGainList, lpGainList, gainAllPass, delay,
                      samplingRate);
}
...
void Reverb::ConstructorCommon(Envelope *percentReverbinput, float *comb_gain_list,
    ...
    // Find the decay time
    float alpha = 0.0;  // alpha is the steady state gain (which is also the max
                        // of the gain fn
#define max(x, y) ((x) > (y) ? (x) : (y))
    for (i = 0; i < 6; i++) alpha = max(alpha, comb_gain_list[i] / (1.0 - lp_gain_list[i]));
...
Reverb::~Reverb() {
    ...
    for (i = 0; i < REVERB_NUM_COMB_FILTERS; i++) delete lpcfilter[i];
...
void Reverb::reset(void) {
    ...
    for (i = 0; i < REVERB_NUM_COMB_FILTERS; i++) lpcfilter[i]->reset();
    apfilter->reset();
}
 ...
 **/
Track &Reverb::do_reverb_Track(Track &inWave) { return do_reverb_Track(inWave, percentReverb); }
...
SoundSample *Reverb::do_reverb_SoundSample(SoundSample *inWave, Envelope *percentReverbinput) {
    ...
    for (i = 0; i < inWave->getSampleCount(); i++)
        (*outWave)[i] = do_reverb((*inWave)[i], (float)i / inWave->getSampleCount(), percentReverb);
...
float Reverb::getDecay(void) {
    // this value used to be multiplied by 4 for some reason that I can't
    // fathom.  this was the cause of a multitude of extra silence at the end of
    // the file, so I killed it.
    //  -AL
 ...
 *
 **/
Envelope *Reverb::getEnvelope(void) { return percentReverb; }
...
SoundSample *Reverb::constructAmp(SoundSample *wave) {
    ...
    for (; ctr1 < nSamples; ctr1++) {
        ...
        if (maxAmpPos <= (ctr1 - windowRadius)) {
            ...
            maxAmp = 0.0;
            for (pos = max(ctr1 - windowRadius, 0); pos < min(ctr1 + windowRadius, nSamples);
                 pos++) {
                if ((*wave)[pos] > maxAmp) {
                    maxAmp = (*wave)[pos];
                    maxAmpPos = pos;
...
void Reverb::set_allPassDelay(float new_allPassDelay) { allPassDelay = new_allPassDelay; }

void Reverb::set_decay_duration(float new_decay_duration) { decay_duration = new_decay_duration; }

void Reverb::setLPComb(int idx, LPCombFilter *f) { lpcfilter[idx] = f; }

void Reverb::setAllPass(AllPassFilter *f) { apfilter = f; }
