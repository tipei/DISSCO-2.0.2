...
AllPassFilter::AllPassFilter(float gain, long delay) {
    ...
    for (long j = 0; j < D; j++) y_hist->enqueue(0.0);
}
//----------------------------------------------------------------------------//
...
void AllPassFilter::xml_read(XmlReader::xmltag *apftag) {
    char *value;
    if ((value = apftag->findChildParamValue("g", "value")) != 0) set_g(atof(value));

    if ((value = apftag->findChildParamValue("D", "value")) != 0) set_D(atoi(value));
}
