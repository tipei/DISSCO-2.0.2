...
void Sound::setPartialParam(PartialStaticParam p, m_value_type v) {
    ...
    while (it.hasNext()) it.next().setParam(p, v);
}
...
*/
//----------------------------------------------------------------------------//
MultiTrack* Sound::render(int numChannels, m_rate_type samplingRate)
...
//  .h file is DEFAULT_SAMPLING_RATE
{
    ...
    sampleCount =
        (m_sample_count_type)((m_time_type)getTotalDuration() * (m_time_type)samplingRate);
    ...
    if (reverbObj != NULL) {
        ...
        // delete the temporary track object that held the unspatialized
        // reverbed sound
...
//----------------------------------------------------------------------------//
void Sound::xml_print(ofstream& xmlOutput, list<Reverb*>& revObjs,
    ...
    xmlOutput << "\t\t<detune_fundamental value=\"" << getParam(DETUNE_FUNDAMENTAL) << "\" />"
              << endl;
...
//----------------------------------------------------------------------------//
void Sound::xml_read(XmlReader::xmltag* soundtag, DISSCO_HASHMAP<long, Reverb*>* reverbHash,
    ...
    if ((value = soundtag->findChildParamValue("reverb_ptr", "id")) != 0)
        if (Reverb* temp = (*reverbHash)[atoi(value)]) use_reverb(temp);
