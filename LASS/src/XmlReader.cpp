...
//----------------------------------------------------------------------------//
XmlReader::xmltag::~xmltag() { destroyTag(); }

//----------------------------------------------------------------------------//
XmlReader::tagparam *XmlReader::xmltag::findParam(const char *pname) {
    tagparam *tp = params;
    while (tp != NULL) {
        if (!strcasecmp(tp->name, pname)) return tp;
...
bool XmlReader::fillTagBuffer() {
    ...
    if (!start) {
        while (!start) {
            ...
            if (!fgets(inputbuffer, XML_BUFFER_SIZE - nibuf, fp)) return false;
    ...
    while (!end) {
        ...
        if (!fgets(inputbuffer + nibuf, XML_BUFFER_SIZE - nibuf, fp)) return false;
    ...
    if (start >= end)
        // printf("[%s:%d] FAIL! Start: %X, End: %X, nibuf:
        // %d\n",__FILE__,__LINE__,start,end,nibuf);
        std::cout << "[" << __FILE__ << ":" << __LINE__ << "] FAIL! Start: " << start
                  << ", End: " << end << ", nibuf: " << nibuf << std::endl;
...
#ifdef XML_DEBUG
                        printf(
                            "Warning: No closequote found on parameter for "
                            "%s\n",
                            n);
#endif
                    }
